...
def _extract_from_meta(page):
    ...
    for meta in page.findAll("meta"):
        ...
        if (
            meta_name in DATE_META_NAMES
            or item_prop in DATE_META_NAMES
            or meta_property in DATE_META_NAMES
            or http_equiv == "date"
        ):
...
def _extract_from_tags(page):
    ...
    for tag in page.find_all(["span", "p", "div"], class_=re.compile(regex, re.IGNORECASE)):
