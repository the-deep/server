        ...
        if isinstance(value, dict):
            ...
            if isinstance(value[0], str):
                        ...
                        _CustomErrorType(
                            ...
                            array_errors=[
                                ArrayNestedErrorType(
                                    client_id=ARRAY_NON_MEMBER_ERRORS, messages="".join(str(msg) for msg in value)
                                )
                            ],
                        )
                    )
                ...
                for pos, array_item in enumerate(value):
                    ...
                    array_errors.append(
                        ArrayNestedErrorType(
                            client_id=client_id, object_errors=serializer_error_to_error_types(array_item, initial_data_field_pos)
                        )
                    )
                error_types.append(
                    _CustomErrorType(client_id=node_client_id, field=_camelize_django_str(field), array_errors=array_errors)
                )
        else:
            # fallback
            error_types.append(_CustomErrorType(field=_camelize_django_str(field), messages=" ".join(str(msg) for msg in value)))
    return error_types
