...
class UserGroupApiTest(TestCase):
    def test_create_user_group(self):
        ...
        self.assertEqual(response.data["memberships"][0]["member"], self.user.pk)
    ...
    def test_usergoup_fields(self):
        ...
        self.create(UserGroup, role="admin", created_by=user)
        url = "/api/v1/user-groups/"
        self.authenticate()
    ...
    def test_member_of(self):
        ...
        self.assertEqual(
            set([user_group["id"] for user_group in response.data["results"]]), set([user_group1.id, user_group2.id])
        )
    ...
    def test_search_user_group_without_exclusion(self):
        ...
        ProjectUserGroupMembership.objects.create(project=project, usergroup=user_group1)
        ...
        data = response.json()
        assert data["count"] == 2
        assert data["results"][0]["id"] == user_group1.id, "'MyTestUserGroup' matches more to search query 'test'"
    ...
    def test_search_user_group_with_exclusion(self):
        ...
        user_group1 = self.create(UserGroup, title="MyTestUserGroup")
        user_group2 = self.create(UserGroup, title="MyUserTestGroup")
        url = "/api/v1/user-groups/?search=test&members_exclude_project=" + str(project.id)
    ...
    def test_add_member(self):
        ...
        ProjectUserGroupMembership.objects.create(usergroup=user_group, project=project)
    ...
    def test_user_group_memberships(self):
        ...
        self.assertEqual(set(members["member"] for members in response.data["results"]), set([test_user1.id, test_user2.id]))
