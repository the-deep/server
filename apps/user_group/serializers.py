...
class GroupMembershipSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    member_email = serializers.CharField(source="member.email", read_only=True)
    member_name = serializers.SerializerMethodField()

    class Meta:
        model = GroupMembership
        fields = ("id", "member", "member_name", "member_email", "group", "role", "joined_at")
...
class UserGroupSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, UserResourceSerializer):
    ...
    def create(self, validated_data):
        ...
        GroupMembership.objects.create(group=user_group, member=self.context["request"].user, role="admin")
        return user_group
    ...
    def get_role(self, user_group):
        ...
        membership = GroupMembership.objects.filter(group=user_group, member=user).first()
        if membership:
...
class GroupMembershipGqSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = GroupMembership
        fields = (
            "id",
...
class UserGroupGqSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, UserResourceSerializer):
    class Meta:
        model = UserGroup
        fields = (
            "id",
    ...
    def update(self, instance, validated_data):
        ...
        if user_group.created_by and not user_group.members.filter(pk=user_group.created_by_id).exists():
            GroupMembership.objects.create(group=user_group, member=user_group.created_by, role="admin")
        return user_group
