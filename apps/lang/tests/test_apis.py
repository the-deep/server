...
from lang.models import Link, LinkCollection, String
...
class LangApiTests(TestCase):
    def test_update_language(self):
        ...
        l1 = Link.objects.filter(link_collection=group1, key="l_1", string=s1).first()
        l2 = Link.objects.filter(link_collection=group1, key="l_2", string=s2).first()
        l3 = Link.objects.filter(link_collection=group2, key="l_1", string=s2).first()
