...
from deep.writable_nested_serializers import NestedCreateMixin, NestedUpdateMixin


class UserResourceBaseSerializer(serializers.Serializer):
    modified_at = serializers.DateTimeField(read_only=True)
    modified_by = serializers.PrimaryKeyRelatedField(read_only=True)
    created_by_name = serializers.CharField(source="created_by.profile.get_display_name", read_only=True)
    modified_by_name = serializers.CharField(source="modified_by.profile.get_display_name", read_only=True)
...
class UserResourceSerializer(UserResourceBaseSerializer, WritableNestedModelSerializer):
    ...
    def _prefetch_related_instances(self, field, related_data):
        ...
        instances = {str(related_instance.pk): related_instance for related_instance in qs.filter(pk__in=pk_list)}
