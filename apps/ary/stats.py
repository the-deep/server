...
def _get_ary_field_options(config):
    ...
    if field_type == "metadatafield":
        return list(MetadataField.objects.get(pk=pk).options.values("key", "title").values(id=F("key"), name=F("title")))
    elif field_type == "methodologyfield":
        return list(MethodologyField.objects.get(pk=pk).options.values("key", "title").values(id=F("key"), name=F("title")))
    elif field_type == "scorepillar":
        return list(ScorePillar.objects.get(pk=pk).questions.values("id", name=F("title")))
...
def get_project_ary_entry_stats(project):
    ...
    dynamic_meta = {key: _get_ary_field_options(value) for key, value in dynamic_fields.items() if value.get("pk")}

    analytical_density_scale = {scale["id"]: scale["value"] for scale in dynamic_meta["analytical_density"]["scale"]}
    ...
    static_meta = {
        ...
        "organization_type": list(
            ...
            ).values("id", "organization_count", name=F("title"))
        ),
        "organization": [
            {
                "id": org.id,
    ...
    for ary in Assessment.objects.prefetch_related("lead", "lead__attachment").filter(project=project).all():
        ...
        scores = {
            ...
            **{
                ...
                score_matrix_pillar_key: {
                    ...
                    for sector_id, scale_id in (
                        matrix_pillars.get(str(dynamic_fields[score_matrix_pillar_key]["pk"])) or {}
                    ).items()
                }
            ...
            **{
                ...
                scorepillar_key: {
                    option["id"]: (pillars.get(str(dynamic_fields[scorepillar_key]["pk"])) or {}).get(str(option["id"]))
        ...
        lead_source_data = {}
        if (
            lead.source_type in [Lead.SourceType.DISK, Lead.SourceType.DROPBOX, Lead.SourceType.GOOGLE_DRIVE]
            and lead.attachment
            and lead.attachment.file
        ):
        ...
        ary_data = {
            ...
            **{
                unit_of_analysis_type: sum(
                    ...
                    if int(attribute.get(str(dynamic_fields["data_collection_technique"]["pk"])) or -1)
            ...
            "data_collection_technique_sample_size": {
                technique["id"]: sum(
                    attribute.get(str(methodology_attributes_fields["sampling_size_field_pk"])) or 0
                    for attribute in methodology_attributes
                    if int(attribute.get(str(dynamic_fields["data_collection_technique"]["pk"])) or -1) == int(technique["id"])
                )
                for technique in dynamic_meta["data_collection_technique"]
            },
            # Methodology Fields Data
            **{
                key: [attribute.get(str(dynamic_fields[selector]["pk"])) for attribute in methodology_attributes]
                for key, selector in (
                    # NOTE: Make sure the keys are not conflicting with outer keys
