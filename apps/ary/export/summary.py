...
def get_methodology_summary(assessment):
    ...
    attributes = {}
    groups_options = {group.title: MethodologyOption.objects.filter(field__in=group.fields.all()) for group in groups}
    ...
    for attr in methodology.get("Attributes") or []:
        for group, options in groups_options.items():
            ...
            for option in options:
                attr_data[option.title] = attr_data.get(option.title, 0) + (1 if data[0].get("value") == option.title else 0)
...
def get_assessment_export_summary(assessment, planned_assessment=False):
    ...
    processed_affected_groups = [{"title": " and ".join(x["parents"][::-1]), "id": x["id"]} for x in all_affected_groups]
    ...
    planned_assessment_info = {
        **methodology_summary,
        ...
        "focuses": {x: 1 if x in selected_focuses else 0 for x in focuses},
        "sectors": {x: 1 if x in selected_sectors else 0 for x in sectors},
        "protection_information_management": {
            x: 1 if x in selected_methodology_protection_informations else 0 for x in methodology_protection_informations
        },
        "affected_groups": {x["title"]: 1 if x["id"] in selected_affected_groups_ids else 0 for x in processed_affected_groups},
    }
        ...
        "stakeholders": (
            {
                ...
                **{
                    x["schema"]["name"]: len(x["value"]) if x["schema"]["type"] == MetadataField.MULTISELECT else 1
                    for x in other_orgs
                },
            }
            if stakeholders
