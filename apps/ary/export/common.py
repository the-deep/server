...
def combine_dicts(dict_list):
    return _combine_dicts([{_dict["schema"]["name"]: _dict} for _dict in dict_list])
...
def get_name_values(data_dict, keys):
    if not isinstance(keys, list):
        keys = [keys]
    return {x["schema"]["name"]: x["value"] for key in keys for x in data_dict.get(key, [])}
...
def get_name_values_options(data_dict, keys):
    if not isinstance(keys, list):
        keys = [keys]
    return {
        x["schema"]["name"]: {"value": x["value"], "options": x["schema"]["options"]}
        for key in keys
...
def populate_with_all_values(d, key, default=None):
    ...
    options = {v: 0 for k, v in d.get(key, {}).get("options", {}).items()}
    return {**options, **{x: 1 for x in d.get(key, {}).get("value", default)}}
...
def get_assessment_meta(assessment):
    ...
    return {
        ...
        "background": {
            ...
            "crisis_start_date": (
                assessment.bg_crisis_start_date.strftime("%d-%m-%Y")
                if assessment.bg_crisis_start_date
        ...
        "dates": {
            "data_collection_start_date": (
                assessment.data_collection_start_date.strftime("%d-%m-%Y")
                if assessment.data_collection_start_date
                else assessment.data_collection_start_date
            ),
            "data_collection_end_date": (
                assessment.data_collection_end_date.strftime("%d-%m-%Y")
                if assessment.data_collection_end_date
                else assessment.data_collection_end_date
            ),
            "publication_date": (
                assessment.publication_date.strftime("%d-%m-%Y") if assessment.publication_date else assessment.publication_date
            ),
        },
    }
