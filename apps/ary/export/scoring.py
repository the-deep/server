...
def get_scoring(assessment):
    ...
    pillars_final_scores = {
        "{} Final Score".format(title): score for title, score in (scoring_data.get("final_pillars_score") or {}).items()
    }

    matrix_pillars_final_scores = scoring_data.get("matrix_pillars_final_score") or {}

    pillars = {
        pillar: {sub_pillar: sp_data["value"] for sub_pillar, sp_data in pillar_data.items()}
