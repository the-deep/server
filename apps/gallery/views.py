...
from utils.extractor.formats import ods, xlsx
...
class DeprecatedPrivateFileView(views.APIView):
    ...
    def get(self, request, uuid=None, filename=None):
        ...
        if (
            ...
            or Entry.get_for(user).filter(image=file).exists()
            or Entry.get_for(user)
            .filter(
...
def filter_files_by_projects(qs, name, value):
    ...
    return qs.filter(models.Q(projects__in=value) | models.Q(lead__project__in=value))
...
class FileExtractionTriggerView(views.APIView):
    ...
    def post(self, request, version=None):
        ...
        if not settings.TESTING:
            transaction.on_commit(lambda: extract_from_file.delay(file_preview.id))
...
class MetaExtractionView(views.APIView):
    ...
    def get(self, request, file_id=None, version=None):
        ...
        if file_type is None:
            raise exceptions.ValidationError({"file_type": "file_type should be present"})
