...
class FileSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, UserResourceSerializer):
    ...
    def validate_file(self, file):
        ...
        if (
            file.content_type not in deep_doc_types.DEEP_SUPPORTED_MIME_TYPES
            and extension not in deep_doc_types.DEEP_SUPPORTED_EXTENSIONS
        ):
            raise serializers.ValidationError("Unsupported file type {}".format(file.content_type))
        return file
    ...
    def create(self, validated_data):
        ...
        try:
            validated_data["metadata"] = self._get_metadata(validated_data.get("file"))
        except Exception:
...
class GoogleDriveFileSerializer(FileSerializer):
    ...
    def create(self, validated_data):
        ...
        # TODO: is this good?
        validated_data["file"] = InMemoryUploadedFile(file, None, title, mime_type, None, None)
...
class FilePreviewSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = FilePreview
