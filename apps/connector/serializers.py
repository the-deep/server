...
from .models import Connector, ConnectorProject, ConnectorSource, ConnectorUser
from .sources.store import source_store


class SourceOptionSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.Serializer):
    key = serializers.CharField()
    field_type = serializers.CharField()
    title = serializers.CharField()
    options = serializers.ListField(serializers.DictField(serializers.CharField))
...
class SourceDataSerializer(RemoveNullFieldsMixin, serializers.ModelSerializer):
    ...
    def get_existing(self, lead):
        ...
        return check_if_url_exists(lead.url, self.context["request"].user, self.context.get("project"))


class ConnectorUserSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    ...
    class Meta:
        model = ConnectorUser
        fields = ("id", "user", "display_name", "email", "connector", "role", "added_at")
...
class ConnectorProjectSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    title = serializers.CharField(source="project.title", read_only=True)

    class Meta:
        model = ConnectorProject
        fields = ("id", "project", "title", "connector", "role", "added_at")
...
class ConnectorSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, UserResourceSerializer):
    ...
    def get_role(self, connector):
        ...
        usership = ConnectorUser.objects.filter(connector=connector, user=user).first()
        if usership:
