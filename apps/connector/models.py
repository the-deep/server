...
class Connector(UserResource):
    ...
    users = models.ManyToManyField(User, blank=True, through="ConnectorUser")
    projects = models.ManyToManyField(Project, blank=True, through="ConnectorProject")
    ...
    def get_for(user):
        return Connector.objects.filter(
            models.Q(users=user) | models.Q(projects__members=user) | models.Q(projects__user_groups__members=user)
        ).distinct()
...
class ConnectorUser(models.Model):
    ...
    role = models.CharField(max_length=96, choices=ROLES, default="normal")
...
class ConnectorProject(models.Model):
    ...
    connector = models.ForeignKey(Connector, on_delete=models.CASCADE)
    role = models.CharField(max_length=96, choices=ROLES, default="self")
    added_at = models.DateTimeField(auto_now_add=True)
