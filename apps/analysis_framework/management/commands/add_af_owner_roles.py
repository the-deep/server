from analysis_framework.models import AnalysisFramework as AF
from analysis_framework.models import AnalysisFrameworkMembership as AFMembership
from analysis_framework.models import AnalysisFrameworkRole as AFRole
...
def add_owner_memberships_to_existing_frameworks():
    ...
    for af in AF.objects.all():
        if not AFMembership.objects.filter(framework=af, member=af.created_by).exists():
            ...
            AFMembership.objects.create(framework=af, member=af.created_by, joined_at=af.created_at, role=owner_role)
