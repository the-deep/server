...
def get_nested_filters(original_widget, widget):
    ...
    if hasattr(widget_module, "get_filters"):
        ...
        return [
            {
                **filter,
                "title": "{} - {}".format(original_widget.title, filter.get("title", widget.title)),
                "key": "{}-{}".format(
                    original_widget.key,
...
def get_exportable(widget, data):
    ...
    return {
        "excel": {"type": "nested", "title": widget.title, "children": [e.get("excel") for e in exportables if e.get("excel")]},
        "report": {
            "levels": [level for e in exportables if e.get("report", {}).get("levels") for level in e["report"]["levels"]]
        },
    }
