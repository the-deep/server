...
from analysis_framework.models import AnalysisFramework, AnalysisFrameworkMembership
...
class AnalysisFrameworkTests(TestCase):
    ...
    def test_get_more_memberships_data(self):
        ...
        framework = self.create(AnalysisFramework)
        framework.add_member(user=user1, role=framework.get_or_create_owner_role(), added_by=user2)
    ...
    def test_clone_analysis_framework_without_name(self):
        ...
        project = self.create(Project, analysis_framework=analysis_framework, role=self.admin_role)

        url = "/api/v1/clone-analysis-framework/{}/".format(analysis_framework.id)
        data = {
            "project": project.id,
        }
    ...
    def test_clone_analysis_framework(self):
        ...
        self.assertEqual(response.data["title"], cloned_title)
    ...
    def test_post_framework_memberships(self):
        ...
        data = {"role": framework.get_or_create_owner_role().id, "member": user2.id, "framework": framework.id}
    ...
    def test_project_analysis_framework(self):
        ...
        url = "/api/v1/projects/{}/analysis-framework/".format(project.id)
