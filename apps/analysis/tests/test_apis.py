...
class TestAnalysisAPIs(TestCase):
    ...
    def test_create_pillar_along_with_statement(self):
            ...
            "analytical_statements": [
                {
                    ...
                    "analytical_entries": [
                        ...
                        {"order": 2, "client_id": "1-2", "entry": entry2.id},
                    ],
                },
                {
                    "statement": "test",
        ...
        self.assert_201(response)
        self.assertEqual(AnalysisPillar.objects.count(), pillar_count + 1)
        self.assertEqual(AnalyticalStatement.objects.filter(analysis_pillar__analysis=analysis).count(), statement_count + 2)
            ...
            "analytical_statements": [
                {
                    ...
                    "analytical_entries": [
                        ...
                        {"order": 2, "client_id": "2-1-2", "entry": entry2.id},
                    ],
                },
            ],
        }
        ...
        self.assertEqual(response.data["analytical_statements"][0]["statement"], data["analytical_statements"][0]["statement"])
        ...
        self.assertIn(
            ...
            list(AnalyticalStatement.objects.filter(analysis_pillar__analysis=analysis).values_list("id", flat=True)),
        )
        # checking for the entries
        self.assertEqual(
            AnalyticalStatementEntry.objects.filter(analytical_statement__analysis_pillar__analysis=analysis).count(),
            entry_count + 2,
        )
    ...
    def test_create_analytical_statement(self):
        ...
        data = {
            "analytical_entries": [{"order": 1, "client_id": "1", "entry": entry.id}],
    ...
    def test_version_change_upon_changes_in_analytical_statement(self):
        ...
        data = {
            ...
            "title": "Some title",
            "analytical_statements": [
                {"statement": "coffee", "order": 1, "client_id": "1"},
            ],
        }
        self.authenticate(user)
        response = self.client.post(url, data)
        ...
        data = {
            ...
            "title": "Some title",
            "analytical_statements": [
                {"id": statement_id, "statement": "tea", "order": 1, "client_id": "123"},
            ],
        }
            ...
            "analytical_statements": [
                {
                    ...
                    "analytical_entries": [
                        ...
                        {"order": 2, "client_id": "2", "entry": entry2.id},
                    ],
                },
                {
                    "statement": "test",
    ...
    def test_summary_for_analysis(self):
        ...
        analysis1 = self.create(
            Analysis, title="Test Analysis", team_lead=user, project=project, end_date=now + relativedelta(days=4)
        )
        analysis2 = self.create(
            Analysis, title="Not for test", team_lead=user, project=project, end_date=now + relativedelta(days=7)
        )
        ...
        DiscardedEntry.objects.create(analysis_pillar=pillar1, entry=entry3, tag=DiscardedEntry.TagType.REDUNDANT)
        DiscardedEntry.objects.create(analysis_pillar=pillar1, entry=entry9, tag=DiscardedEntry.TagType.REDUNDANT)
        ...
        DiscardedEntry.objects.create(analysis_pillar=pillar2, entry=entry5, tag=DiscardedEntry.TagType.REDUNDANT)
        ...
        DiscardedEntry.objects.create(analysis_pillar=pillar3, entry=entry2, tag=DiscardedEntry.TagType.REDUNDANT)
        ...
        DiscardedEntry.objects.create(analysis_pillar=pillar4, entry=entry8, tag=DiscardedEntry.TagType.REDUNDANT)
        ...
        self.assertEqual(data[1]["pillars"][2]["assignee_details"]["display_name"], pillar1.assignee.profile.get_display_name())
        self.assertEqual(
    ...
    def test_clone_analysis(self, client_id_mock_func):
        ...
        analytical_statement = self.create(
            AnalyticalStatement, analysis_pillar=pillar, statement="Hello from here", client_id="1"
        )
        self.create(AnalyticalStatementEntry, analytical_statement=analytical_statement, entry=entry, order=1, client_id="1")
        self.create(DiscardedEntry, entry=entry1, analysis_pillar=pillar, tag=DiscardedEntry.TagType.REDUNDANT)
        ...
        self.assertEqual(response.data["end_date"], [ErrorDetail(string="This field is required.", code="required")])

        # try to post with start_date greater than end_date
        data = {"title": "cloned_title", "end_date": "2020-10-01", "start_date": "2020-10-20"}
        self.authenticate(user)
        response = self.client.post(url, data)
        self.assert_400(response)
        self.assertEqual(response.data["end_date"], [ErrorDetail(string="End date must occur after start date", code="invalid")])
        ...
        self.assertEqual(Analysis.objects.count(), 2)  # need to be cloned and created by user
        self.assertEqual(AnalysisPillar.objects.count(), 2)
        self.assertEqual(AnalyticalStatement.objects.count(), 2)
        self.assertEqual(AnalyticalStatementEntry.objects.count(), 2)
        self.assertEqual(DiscardedEntry.objects.count(), 2)
    ...
    def test_patch_analytical_statement(self):
        ...
        statement_entry1 = self.create(
            AnalyticalStatementEntry, analytical_statement=analytical_statement, entry=entry1, order=1, client_id="1"
        )
        statement_entry2 = self.create(
            AnalyticalStatementEntry, analytical_statement=analytical_statement, entry=entry2, order=2, client_id="2"
        )
            ...
            "analytical_statements": [
                {
                    ...
                    "analytical_entries": [
                        ...
                        {"id": statement_entry2.pk, "order": 3, "entry": entry2.id},
                    ],
                }
            ]
        }
    ...
    def test_analysis_overview_in_project(self):
        ...
        self.create(AnalyticalStatementEntry, analytical_statement=analytical_statement1, entry=entry1)
        self.create(AnalyticalStatementEntry, analytical_statement=analytical_statement2, entry=entry2)
        DiscardedEntry.objects.create(analysis_pillar=pillar1, entry=entry4, tag=DiscardedEntry.TagType.REDUNDANT)
        ...
        self.assertIn(organization_type1.title, [item["organization_type_title"] for item in data["authoring_organizations"]])
    ...
    def test_post_discarded_entries_in_analysis_pillar(self):
        ...
        data = {"entry": entry.id, "tag": DiscardedEntry.TagType.REDUNDANT}
        ...
        data = {"entry": entry1.id, "tag": DiscardedEntry.TagType.REDUNDANT}
        url = f"/api/v1/analysis-pillar/{pillar1.id}/discarded-entries/"
        self.authenticate(user2)
        response = self.client.post(url, data)
        self.assert_403(response)
        ...
        data = {"entry": entry2.id, "tag": DiscardedEntry.TagType.REDUNDANT}
        url = f"/api/v1/analysis-pillar/{pillar1.id}/discarded-entries/"
        self.authenticate(user)
        response = self.client.post(url, data)
        self.assert_400(response)
    ...
    def test_all_entries_in_analysis_pillar(self):
        ...
        self.authenticate(user)
        data = {"exclude_entries": [entry2.id, entry3.id]}
    ...
    def test_discardedentry_options(self):
        ...
        response = self.client.get(url)
        self.assert_200(response)
        self.assertEqual(response.data[0]["key"], DiscardedEntry.TagType.REDUNDANT)
        self.assertEqual(response.data[0]["value"], DiscardedEntry.TagType.REDUNDANT.label)
        self.assertEqual(response.data[1]["key"], DiscardedEntry.TagType.TOO_OLD)
        self.assertEqual(response.data[1]["value"], DiscardedEntry.TagType.TOO_OLD.label)
