...
class TestAnalysisNlpMutationSchema(GraphQLTestCase):
    ...
    def test_topic_model(self, trigger_results_mock, RequestHelperMock):
        ...
        def _mutation_check(minput, **kwargs):
            return self.query_check(
                self.TRIGGER_TOPIC_MODEL, minput=minput, mnested=["project"], variables={"projectId": self.project.id}, **kwargs
            )
        ...
        assert _query_check(a_summary_id)["data"]["project"]["analysisTopicModel"]["status"] == self.genum(
            TopicModel.Status.STARTED
        )
        ...
        with self.captureOnCommitCallbacks(execute=True):
            ...
            a_summary_id = response["data"]["project"]["triggerAnalysisTopicModel"]["result"]["id"]
        assert _query_check(a_summary_id)["data"]["project"]["analysisTopicModel"]["status"] == self.genum(
            TopicModel.Status.SEND_FAILED
        )

        topic_model = TopicModel.objects.get(pk=a_summary_id)
        # Check if generated entries are within the project
        assert list(topic_model.get_entries_qs().values_list("id", flat=True)) == [entry.id for entry in lead2_entries]

        # -- Callback test (Mocking NLP part)
        SAMPLE_TOPIC_MODEL_RESPONSE = {
            "cluster_1": {
                "entry_id": [entry.id for entry in lead2_entries[:1]],
                "label": "Label 1",
            },
            "cluster_2": {"entry_id": [entry.id for entry in lead2_entries[1:]], "label": "Label 2"},
        }
    ...
    def test_automatic_summary(self, trigger_results_mock, RequestHelperMock):
        ...
        minput["entriesId"] = [
            str(entry.id) for entries in [lead1_entries, lead2_entries, another_lead_entries] for entry in entries
        ]
        minput["widgetTags"] = [
            "tag1",
            "tag2",
        ...
        assert _query_check(a_summary_id)["data"]["project"]["analysisAutomaticSummary"]["status"] == self.genum(
            AutomaticSummary.Status.STARTED
        )
        ...
        with self.captureOnCommitCallbacks(execute=True):
            ...
            a_summary_id = response["data"]["project"]["triggerAnalysisAutomaticSummary"]["result"]["id"]
        assert _query_check(a_summary_id)["data"]["project"]["analysisAutomaticSummary"]["status"] == self.genum(
            AutomaticSummary.Status.SEND_FAILED
        )
        ...
        a_summary.created_at = self.PATCHER_NOW_VALUE - datetime.timedelta(hours=AutomaticSummary.CACHE_THRESHOLD_HOURS + 1)
        a_summary.save()
    ...
    def test_automatic_ngram(self, trigger_results_mock, RequestHelperMock):
        ...
        assert _query_check(a_ngram_id)["data"]["project"]["analysisAutomaticNgram"]["status"] == self.genum(
            AnalyticalStatementNGram.Status.STARTED
        )
        ...
        with self.captureOnCommitCallbacks(execute=True):
            ...
            a_ngram_id = response["data"]["project"]["triggerAnalysisAutomaticNgram"]["result"]["id"]
        assert _query_check(a_ngram_id)["data"]["project"]["analysisAutomaticNgram"]["status"] == self.genum(
            AnalyticalStatementNGram.Status.SEND_FAILED
        )
        ...
        assert response_result["unigrams"] == [
            dict(word=word, count=count) for word, count in SAMPLE_NGRAM_RESPONSE["unigrams"].items()
        ]
        assert response_result["bigrams"] == [
            dict(word=word, count=count) for word, count in SAMPLE_NGRAM_RESPONSE["bigrams"].items()
        ]
        assert response_result["trigrams"] == []

        a_ngram.created_at = self.PATCHER_NOW_VALUE - datetime.timedelta(hours=AnalyticalStatementNGram.CACHE_THRESHOLD_HOURS + 1)
        a_ngram.save()
    ...
    def test_geo_location(self, trigger_results_mock, RequestHelperMock):
        ...
        def _mutation_check(minput, **kwargs):
            return self.query_check(
                self.TRIGGER_GEOLOCATION, minput=minput, mnested=["project"], variables={"projectId": self.project.id}, **kwargs
            )
        ...
        assert _query_check(geo_task_id)["data"]["project"]["analysisGeoTask"]["status"] == self.genum(
            AnalyticalStatementGeoTask.Status.STARTED
        )
        ...
        with self.captureOnCommitCallbacks(execute=True):
            ...
            geo_task_id = response["data"]["project"]["triggerAnalysisGeoLocation"]["result"]["id"]
        assert _query_check(geo_task_id)["data"]["project"]["analysisGeoTask"]["status"] == self.genum(
            AnalyticalStatementGeoTask.Status.SEND_FAILED
        )
        ...
        geo_task.created_at = self.PATCHER_NOW_VALUE - datetime.timedelta(
            hours=AnalyticalStatementGeoTask.CACHE_THRESHOLD_HOURS + 1
        )
        geo_task.save()
...
class TestAnalysisReportQueryAndMutationSchema(GraphQLTestCase):
    ...
    def test_mutation_and_query(self):
        ...
        def _create_mutation_check(minput, **kwargs):
            return self.query_check(
                self.CREATE_REPORT, minput=minput, mnested=["project"], variables={"projectId": self.project.id}, **kwargs
            )
        ...
        def _query_check(_id, **kwargs):
            return self.query_check(
                self.QUERY_REPORT,
                variables={"projectId": self.project.id, "reportId": _id},
                **kwargs,
            )
        ...
        # --- member user
        for user in [self.readonly_member_user, self.member_user]:
