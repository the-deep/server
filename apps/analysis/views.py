...
from .filter_set import AnalysisFilterSet, DiscardedEntryFilterSet
from .models import Analysis, AnalysisPillar, AnalyticalStatement, DiscardedEntry
...
class AnalysisViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, url_path="clone", methods=["post"])
    def clone_analysis(self, request, project_id, pk=None, version=None):
        ...
        if input_serializer.is_valid():
            ...
            return response.Response(input_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class AnalysisPillarEntryViewSet(EntryFilterView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(
            project=analysis_pillar.analysis.project, lead__published_on__lte=analysis_pillar.analysis.end_date
        )
...
class AnalyticalStatementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            AnalyticalStatement.objects.filter(analysis_pillar=self.kwargs["analysis_pillar_id"])
            .select_related(
                "analysis_pillar",
            )
            .prefetch_related(
