...
from .enums import ExportDataTypeEnum, ExportFormatEnum, ExportStatusEnum
...
class ExportFilterSet(django_filters.rest_framework.FilterSet):
    ...
    status = django_filters.MultipleChoiceFilter(choices=Export.Status.choices, widget=django_filters.widgets.CSVWidget)

    type = django_filters.MultipleChoiceFilter(choices=Export.DataType.choices, widget=django_filters.widgets.CSVWidget)
    exported_at__lt = django_filters.DateFilter(field_name="exported_at", lookup_expr="lte", input_formats=["%Y-%m-%d%z"])
    exported_at__gte = django_filters.DateFilter(field_name="exported_at", lookup_expr="gte", input_formats=["%Y-%m-%d%z"])
