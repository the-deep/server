...
class Run:
    ...
    def add_oval_shape(self, fill_hex_color=None):
        ...
        oval = oxml_parser.makeelement("{%s}%s" % (nsmap["v"], "oval"), attrib=oval_attrs, nsmap=nsmap)

        border_attrs = dict(color="gray", joinstyle="round", endcap="flat")
        stroke = oxml_parser.makeelement("{%s}%s" % (nsmap["v"], "stroke"), attrib=border_attrs, nsmap=nsmap)
        oval.append(stroke)
        pict.append(oval)
...
class Paragraph:
    ...
    def justify(self):
        self.ref.paragraph_format.alignment = docx.enum.text.WD_ALIGN_PARAGRAPH.JUSTIFY
        return self
    ...
    def add_image(self, image):
        try:
            ...
            try:
                ...
                width = (sec.page_width / cols) - (sec.right_margin + sec.left_margin)
            except Exception:
