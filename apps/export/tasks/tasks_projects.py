...
from project.models import ProjectMembership, ProjectOrganization, ProjectRole
...
def get_organizations_display(project, organization_type=None):
    ...
    return ",".join(
        [org.data.title for org in Organization.objects.filter(id__in=organization_ids_qs.values_list("organization", flat=True))]
    )
