...
from ary.export.affected_groups_info import (
    get_affected_groups_info as ary_get_affected_groups_info,
)
...
from entry.models import Attribute, Entry, ExportData  # EntryGroupLabel,
...
from utils.common import deep_date_format, deep_date_parse
...
class WidgetExporter:
    ...
    def _get_date_range_widget_data(cls, data, bold, **kwargs):
        ...
        if len(values) == 2 and any(values):
            label = "{} - {}".format(
                date_renderer(
                    deep_date_parse(values[0], raise_exception=False),
                    fallback="N/A",
                ),
                date_renderer(
                    deep_date_parse(values[1], raise_exception=False),
                    fallback="N/A",
                ),
            )
    ...
    def _get_organigram_widget_data(cls, data, bold, **kwargs):
        text = INTERNAL_SEPARATOR.join("/".join(value_with_parent) for value_with_parent in data.get("values"))
        return cls._add_common, text, bold
...
class ReportExporter:
    def __init__(
        ...
        self.doc = Document(os.path.join(settings.APPS_DIR, "static/doc_export/template.docx"))
        self.lead_ids = []
        # ordered list of widget ids
        self.exporting_widgets_ids = [int(_id) for _id in exporting_widgets or []]
        self.exporting_widgets_keys = list(Widget.objects.filter(id__in=self.exporting_widgets_ids).values_list("key", flat=True))
        self.region_data = {}
    ...
    def load_exportables(self, exportables, regions):
        ...
        geo_data_required = Widget.objects.filter(id__in=self.exporting_widgets_ids, widget_id=geo_widget.WIDGET_ID).exists()
    ...
    def load_text_from_text_widgets(self, entries, text_widget_ids):
        ...
        widget_map = {int(id): index for index, id in enumerate(text_widget_ids)}
            ...
            if raw_export_data:
                for data in raw_export_data:
                    try:
                        ...
                        logger.error(
                            f"ExportDataVersionMismatch: For entry {entry.id}, project {entry.project.id}", exc_info=True
                        )
    ...
    def _generate_for_entry(self, entry):
        ...
        excerpt = entry.excerpt if entry.entry_type == Entry.TagType.EXCERPT else ""
    ...
    def _generate_for_levels(
        ...
        for level in levels:
            ...
            if sublevels:
                substructures = None
                if structures:
                    substructures = next((s.get("levels") for s in structures if s["id"] == level.get("id")), None)
    ...
    def _generate_for_uncategorized(self, entries, categorized_entry_processed):
        entries = entries.exclude(Q(exportdata__data__report__keys__isnull=False) | Q(exportdata__data__report__keys__len__gt=0))
    ...
    def add_entries(self, entries):
        ...
        for exportable in exportables:
            levels = (
                ...
                af_levels_map.get(str(exportable.pk))
                or
            ...
            for entry in iterable_entries:
                ...
                if export_data:
                    self._load_into_levels(
                        entry,
                        export_data.data.get("report").get("keys"),
                        levels,
                        level_entries_map,
                        valid_levels,
                    )
            ...
            structures = self.structure and next(
                (s.get("levels") for s in self.structure if str(s["id"]) == str(exportable.id)), None
            )
            self._generate_for_levels(levels, level_entries_map, valid_levels, structures)
