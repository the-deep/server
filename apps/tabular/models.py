...
class Book(UserResource):
    ...
    META_REQUIRED_FILE_TYPES = [XLSX, XLS]
    ...
    def get_processed_fields(self, fields=[]):
        ...
        return Field.objects.filter(
            sheet__book=self,
            cache__status=Field.CACHE_SUCCESS,
            id__in=fields,
        ).distinct()
...
class Sheet(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.data_row_index != self.current_data_row_index:
            from tabular.tasks import (
                tabular_generate_columns_image,  # to prevent circular import
            )
            ...
            transaction.on_commit(lambda: tabular_generate_columns_image.delay(list(field_ids)))
...
class Field(models.Model):
    ...
    type = models.CharField(max_length=30, choices=FIELD_TYPES, default=STRING)
    hidden = models.BooleanField(default=False)
    options = models.JSONField(default=None, blank=True, null=True)
    ...
    def cast_data(self, geos_names={}, geos_codes={}):
        ...
        if type == Field.GEO and regions:
            options["regions"] = [{"id": k, "title": v} for k, v in regions.items()]
...
class Geodata(models.Model):
    ...
    field = models.OneToOneField(Field, on_delete=models.CASCADE, related_name="geodata")
    status = models.CharField(
        max_length=30,
        choices=STATUS_TYPES,
        default=PENDING,
