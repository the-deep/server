...
def trigger_cache_reset(modeladmin, request, queryset):
    messages.add_message(
        ...
        messages.INFO,
        mark_safe(
            "Successfully triggerd fields: <br><hr>"
            + "<br>".join("* {} : {}".format(value[0], value[1]) for value in queryset.values_list("id", "title").distinct())
        ),
    )
    tabular_generate_columns_image.delay(list(queryset.values_list("id", flat=True).distinct()))
