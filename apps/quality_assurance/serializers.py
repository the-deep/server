...
class EntryReviewCommentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if mentioned_users:
            selected_existing_members_count = (
                ProjectMembership.objects.filter(project=entry.project, member__in=mentioned_users).distinct("member").count()
            )
            if selected_existing_members_count != len(mentioned_users):
                raise serializers.ValidationError({"mentioned_users": "Selected mentioned users don't belong to this project"})
    ...
    def comment_save(self, validated_data, instance=None):
        ...
        if (
            not text
            and not (instance and instance.text)
            and comment_type
            in [
        ...
        if instance is None:  # Create
            ...
            notify_meta["new_mentioned_users"] = [
                user for user in validated_data.get("mentioned_users", []) if user.pk not in current_mentioned_users_pk
            ]
            instance = super().update(instance, validated_data)

        if text and text_changed:
            self._add_comment_text(instance, text)
        transaction.on_commit(lambda: send_notifications_for_comment(instance.pk, notify_meta))
        return instance
...
class EntryReviewCommentGqlSerializer(ProjectPropertySerializerMixin, serializers.ModelSerializer):
    ...
    @cached_property
    def entry(self):
        entry = (self.instance and self.instance.entry) or Entry.objects.filter(id=self.initial_data.get("entry")).first()
        if entry is None:
    ...
    def validate_comment_type(self, comment_type):
        ...
        if comment_type in [
            EntryReviewComment.CommentType.CONTROL,
            EntryReviewComment.CommentType.UNCONTROL,
        ] and not PP.check_permission_from_serializer(
    ...
    def validate_mentioned_users(self, mentioned_users):
        if mentioned_users:
            selected_existing_members_count = (
                ProjectMembership.objects.filter(project=self.project, member__in=mentioned_users).distinct("member").count()
            )
            if selected_existing_members_count != len(mentioned_users):
    ...
    def validate(self, validated_data):
        ...
        comment_type = (
            validated_data.get("comment_type")
            or (self.instance and self.instance.comment_type)
            or EntryReviewComment.CommentType.COMMENT
        )
        # Make sure to check text required
