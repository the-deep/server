...
class TestProjectSchema(GraphQLTestCase):
    ...
    def test_project_query(self):
        ...
        ProjectJoinRequestFactory.create(
            project=public_project, requested_by=request_user, status="pending", role=self.project_role_admin
        )
        # create projectJoinRequest(status='rejected')
        ProjectJoinRequestFactory.create(
            project=public_project4, requested_by=request_user, status="rejected", role=self.project_role_admin
        )
        ...
        EntryFactory.create_batch(4, project=public_project, analysis_framework=analysis_framework, lead=lead)
        entry2_1 = EntryFactory.create(project=public_project, analysis_framework=analysis_framework, lead=lead2, controlled=True)
    ...
    def test_public_projects(self):
        ...
        public_af = AnalysisFrameworkFactory.create(is_private=False, title="Public Analysis Framework Title")
        private_af = AnalysisFrameworkFactory.create(title="Private Analysis Framework Title", is_private=True)
        ...
        self.assertEqual(
            content["data"]["publicProjects"]["results"][0]["analysisFrameworkTitle"], "Public Analysis Framework Title", content
        )
        self.assertEqual(content["data"]["publicProjects"]["results"][2]["analysisFrameworkTitle"], None, content)
        # some check for regions private and public
        self.assertEqual(content["data"]["publicProjects"]["results"][2]["regionsTitle"], "Public Region", content)
        self.assertEqual(content["data"]["publicProjects"]["results"][1]["regionsTitle"], "", content)
    ...
    def test_project_stat_recent(self):
        ...
        EntryFactory.create(
            lead=lead1, controlled=False, created_by=user, project=public_project1, analysis_framework=analysis_framework
        )
    ...
    def test_project_allowed_permissions(self):
        ...
        content_projects_permissions = {int(pdata["id"]): pdata["allowedPermissions"] for pdata in content_projects}
    ...
    def test_projects_by_region(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "id": str(region2.pk),
                    "centroid": {"coordinates": [region2.centroid.x, region2.centroid.y], "type": "Point"},
        ...
        self.assertEqual(
            ...
            [
                ...
                {
                    "id": str(region1.pk),
                    "centroid": {"coordinates": [region1.centroid.x, region1.centroid.y], "type": "Point"},
    ...
    def test_project_stats_with_filter(self):
        ...
        for index, (filters, _expected) in enumerate(
            [
                ({"confidentiality": self.genum(Lead.Confidentiality.CONFIDENTIAL)}, [1, 1, 0, 0, 2, 2, 0]),
                (
                    {"entriesFilterData": {"leadConfidentialities": self.genum(Lead.Confidentiality.CONFIDENTIAL)}},
                    [1, 1, 0, 0, 2, 2, 0],
                ),
                ({"confidentiality": self.genum(Lead.Confidentiality.UNPROTECTED)}, [1, 0, 0, 1, 1, 0, 1]),
                (
    ...
    def test_user_pinned_projects_query(self):
        ...
        for idx, project in enumerate(project_with_access):
            ...
            ProjectPinnedFactory.create(project=project, user=user1, order=idx)
        # -- Without login
        self.query_check(query, assert_for_error=True)
