...
from .models import Project, ProjectJoinRequest, ProjectOrganization
...
class ProjectStatLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        ...
        _map = {project_with_stat.id: project_with_stat for project_with_stat in stat_qs}
...
class ProjectJoinStatusLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        ...
        _map = {project_id: True for project_id in join_status_qs}
...
class ProjectExploreStatsLoader(WithContextMixin):
    def get_stats(self):
        ...
        latest_active_projects_qs = Project.objects.filter(is_private=False).order_by(
            "-stats_cache__entries_activity", "-created_at"
        )
        latest_active_projects = latest_active_projects_qs.values(
        ...
        # Recent frameworks
        top_active_frameworks = (
            AnalysisFramework.objects.filter(is_private=False)
            .annotate(
                project_count=models.functions.Coalesce(
                    models.Subquery(
                        Project.objects.filter(analysis_framework=models.OuterRef("pk"))
                        .order_by()
                ...
                source_count=models.functions.Coalesce(
                    models.Subquery(
                        Lead.objects.filter(project__analysis_framework=models.OuterRef("pk"))
                        .order_by()
                        .values("project__analysis_framework")
            ...
            .order_by("-project_count", "-source_count")
            .values(
        ...
        return dict(
            ...
            leads_added_weekly=leads_count
            and (
                leads_count
                / (((abs(lead_created_at_range["max_created_at"] - lead_created_at_range["min_created_at"]).days) // 7) or 1)
            ),
            daily_average_leads_tagged_per_project=tagged_leads_count
            and (
                tagged_leads_count
                / (
                    (abs(tagged_lead_created_at_range["max_created_at"] - tagged_lead_created_at_range["min_created_at"]).days)
                    or 1
                )
                / (project_count or 1)
            ),
            generated_exports_monthly=exports_count
            and (
                exports_count
                / (
                    get_number_of_months_between_dates(
                        exports_created_at_range["max_exported_at"], exports_created_at_range["min_exported_at"]
                    )
                    or 1
                )
            ),
...
class GeoRegionLoader(DataLoaderWithContext):
    ...
    def batch_load_fn(self, keys):
        qs = (
            self.get_region_queryset()
            .filter(project__in=keys)
            .annotate(
