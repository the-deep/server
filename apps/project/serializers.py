...
class SimpleProjectSerializer(RemoveNullFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = Project
...
class ProjectRoleSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    ...
    def get_lead_permissions(self, roleobj):
        return [k for k, v in PROJECT_PERMISSIONS["lead"].items() if roleobj.lead_permissions & v != 0]

    def get_entry_permissions(self, roleobj):
        return [k for k, v in PROJECT_PERMISSIONS["entry"].items() if roleobj.entry_permissions & v != 0]

    def get_setup_permissions(self, roleobj):
        return [k for k, v in PROJECT_PERMISSIONS["setup"].items() if roleobj.setup_permissions & v != 0]

    def get_export_permissions(self, roleobj):
        return [k for k, v in PROJECT_PERMISSIONS["export"].items() if roleobj.export_permissions & v != 0]

    def get_assessment_permissions(self, roleobj):
        return [k for k, v in PROJECT_PERMISSIONS["assessment"].items() if roleobj.assessment_permissions & v != 0]


class SimpleProjectRoleSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = ProjectRole
        fields = ("id", "title", "level")


class ProjectOrganizationSerializer(
    RemoveNullFieldsMixin, DynamicFieldsMixin, UserResourceSerializer, serializers.ModelSerializer
):
...
class ProjectMembershipSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    member_email = serializers.CharField(source="member.email", read_only=True)
    member_name = serializers.CharField(source="member.profile.get_display_name", read_only=True)
    ...
    def get_member_status(self, membership):
        if ProjectRole.get_admin_roles().filter(id=membership.role.id).exists():
            return "admin"
        return "member"
    ...
    def project_member_validation(self, project, member):
        if ProjectMembership.objects.filter(project=project, member=member).exists():
...
class ProjectUsergroupMembershipSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
...
class ProjectSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, DeprecatedUserResourceSerializer):
    ...
    def create(self, validated_data):
        ...
        private_access = member.profile.get_accessible_features().filter(key=Feature.FeatureKey.PRIVATE_PROJECT).exists()

        if is_private and not private_access:
            raise PermissionDenied({"message": "You don't have permission to create private project"})
    ...
    def update(self, instance, validated_data):
        ...
        if "is_private" in validated_data and validated_data["is_private"] != instance.is_private:
        ...
        raise PermissionDenied({"message": "You don't have permissions to use the analysis framework in the project"})
    ...
    def get_is_visualization_enabled(self, project):
        ...
        entry_viz_enabled = is_viz_enabled and af.properties is not None and af.properties.get("stats_config") is not None
    ...
    def get_member_status(self, project):
        ...
        if join_request and (join_request.status == "pending" or join_request.status == "rejected"):
    ...
    def get_role(self, project):
        ...
        membership = ProjectMembership.objects.filter(project=project, member=user).first()
        if membership:
    ...
    def validate_user_groups(self, user_groups):
        for user_group_obj in self.initial_data["user_groups"]:
            ...
            if not user_group.can_modify(self.context["request"].user):
                raise serializers.ValidationError("Invalid user group: {}".format(user_group.id))
        return user_groups
    ...
    def validate_regions(self, data):
        for region_obj in self.initial_data["regions"]:
            ...
            if self.instance and region in self.instance.regions.all():
                continue
            if not region.public and not region.can_modify(self.context["request"].user):
                raise serializers.ValidationError("Invalid region: {}".format(region.id))
        return data

    def validate_analysis_framework(self, analysis_framework):
        if not analysis_framework.can_get(self.context["request"].user):
            raise serializers.ValidationError("Invalid analysis framework: {}".format(analysis_framework.id))
...
class ProjectStatSerializer(ProjectSerializer):
    ...
    def _get_top_entity_contributer(self, project, Entity):
        contributers = (
            ...
            .select_related("member", "member__profile")[:5]
        )
...
class ProjectJoinRequestSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
...
class ProjectUserGroupSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if usergroup and ProjectUserGroupMembership.objects.filter(project=data["project_id"], usergroup=usergroup).exists():
...
class ProjectJoinGqSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProjectJoinRequest
        fields = ("id", "reason", "role", "requested_by", "responded_by", "project", "status", "data")
    ...
    def create(self, validated_data):
        ...
        transaction.on_commit(lambda: send_project_join_request_emails.delay(instance.id))
        return instance
...
class ProjectAcceptRejectSerializer(serializers.ModelSerializer):
    ...
    def _accept_request(responded_by, join_request, role):
        ...
        transaction.on_commit(lambda: send_project_accept_email.delay(join_request.id))
    ...
    def _reject_request(responded_by, join_request):
        ...
        join_request.responded_at = timezone.now()
        join_request.save()
        transaction.on_commit(lambda: send_project_reject_email.delay(join_request.id))

    def update(self, instance, validated_data):
        validated_data["project"] = self.context["request"].active_project
        role = validated_data.pop("role", None)
        if instance.status in ["accepted", "rejected"]:
            raise serializers.ValidationError("This request has already been {}".format(instance.status))
...
class ProjectMembershipGqlSerializer(TempClientIdMixin, serializers.ModelSerializer):
    ...
    def validate_role(self, new_role):
        ...
        if (
            self.instance  # For Update
            and self.instance.role != new_role  # For changed role
            and (
                self.instance.role.level == self.current_user_role.level  # Requesting user role == current member role
                and self.instance.role.level < new_role.level  # New role is lower then current role
            )
        ):
...
class ProjectUserGroupMembershipGqlSerializer(TempClientIdMixin, serializers.ModelSerializer):
    ...
    def validate_role(self, new_role):
        ...
        if (
            ...
            and self.instance.role != new_role  # Role is changed
            and (
                self.instance.role.level == self.current_user_role.level  # Requesting user role == current member role
                and self.instance.role.level < new_role.level  # New role is lower then current role
            )
...
class ProjectGqSerializer(DeprecatedUserResourceSerializer):
    ...
    def validate_is_private(self, is_private):
        ...
        # For create, make sure user can feature permission to create private project.
            ...
            private_access = self.current_user.profile.get_accessible_features().filter(key=Feature.FeatureKey.PRIVATE_PROJECT)
    ...
    def validate_analysis_framework(self, framework):
        ...
        if not framework.can_get(self.current_user):
            raise serializers.ValidationError("Given framework either doesn't exists or you don't have access to it")
