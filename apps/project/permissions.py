...
PROJECT_PERMISSIONS = Dict(
    ...
    assessment=Dict({"view": 1 << 0, "create": 1 << 1, "modify": 1 << 2, "delete": 1 << 3}),
)
...
class JoinPermission(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        ...
        return (
            not obj.is_member(request.user)
            and not ProjectJoinRequest.objects.filter(
                models.Q(status="pending") | models.Q(status="rejected"),
                project=obj,
...
class MembershipModifyPermission(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        ...
        project = obj.project
        user = ProjectMembership.objects.filter(project=project, member=request.user).first()
...
def get_project_entities(Entity, user, action=None):
    ...
    return (
        ...
        .annotate(new_permission_col=models.F(fieldname).bitand(permission))
        .filter(new_permission_col=permission)
        .distinct()
    )
