...
from .models import Project, ProjectChangeLog, ProjectOrganization


def get_flat_dict_diff(list1: List[dict], list2: List[dict], fields: List[str]):
    def _dict_to_tuple_set(items: List[dict]) -> Set[tuple]:
        return set(tuple(item[field] for field in fields) for item in items)

    def _tuple_to_dict_list(items: Set[tuple]) -> List[dict]:
        return [{field: item[index] for index, field in enumerate(fields)} for item in sorted(items)]
...
class ProjectChangeManager:
    ...
    def get_active_project_latest_data(self):
        return ProjectDataSerializer(Project.objects.get(pk=self.project_id)).data
    ...
    def log_framework_update(cls, af_id, user):
        ...
        """
        project_ids = Project.objects.filter(analysis_framework=af_id).values_list("id", flat=True)
        change_logs = [
            ProjectChangeLog(
                user=user,
                project_id=project_id,
