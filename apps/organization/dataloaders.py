...
class LogoLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        file_qs = File.objects.annotate(organization_id=F("organization")).filter(organization__in=keys)
        _map = {file.organization_id: file for file in file_qs}
