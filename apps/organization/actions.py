    ...
    if request.POST.get("post"):
        ...
        if n:
            ...
            modeladmin.message_user(
                request,
                _("Successfully merged %(count)d %(items)s.") % {"count": n, "items": model_ngettext(modeladmin.opts, n)},
                messages.SUCCESS,
            )
...
def merge_organizations(modeladmin, request, queryset):
    try:
        ...
        messages.add_message(
            request,
            messages.ERROR,
            mark_safe("Error occured while merging organization: <br><hr><pre>" + traceback.format_exc() + "</pre>"),
        )
