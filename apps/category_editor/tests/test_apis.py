...
class CategoryEditorTests(TestCase):
    ...
    def test_clone_category_editor(self):
        ...
        project = self.create(Project, category_editor=category_editor, role=self.admin_role)
        ...
        self.assertEqual(response.data["title"], category_editor.title[:230] + " (cloned)")

        project = Project.objects.get(id=project.id)
        self.assertNotEqual(project.category_editor.id, category_editor.id)
    ...
    def test_classify(self):
        ...
        url = "/api/v1/projects/{}/category-editor/classify/".format(project.id)
        data = {
            "text": text,
            "category": "sector",
        }
        ...
        for g in got:
            g["keywords"] = [dict(k) for k in g["keywords"]]
