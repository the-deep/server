...
from rest_framework import exceptions, permissions, response, status, views, viewsets
...
class CategoryEditorCloneView(views.APIView):
    ...
    def post(self, request, ce_id, version=None):
        if not CategoryEditor.objects.filter(id=ce_id).exists():
            raise exceptions.NotFound()

        category_editor = CategoryEditor.objects.get(id=ce_id)
...
class CategoryEditorClassifyView(views.APIView):
    ...
    def _process_subcategory(self, category, text, results):
        ...
        for _, ngram in ngrams.items():
            [category_results.extend(self._search_word(title, word, text)) for word in ngram if word.lower() in text]
