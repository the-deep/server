...
def get_affected_groups_key(choice_model, label):
    choices = {
        k: v.split('/')[-1]
        for k, v in choice_model.choices
    }
    if not label:
        return
...
def save_stakeholders(metadata_dict, assessment_registry):
    stakeholders_dict = {
        k: v for k, v in AssessmentRegistryOrganization.Type.choices
    }
...
def migrate_score_data(old_ary, assessment_reg):
    ...
    for sector, value in sector_value_dict:
        ...
        if sector_key:
            ScoreAnalyticalDensity.objects.get_or_create(
                assessment_registry=assessment_reg,
                sector=sector_key,
                score=value * 2
            )
                ...
                if v:
                    ...
                    if ass_question:
                        Answer.objects.create(
                            question=ass_question,
                            assessment_registry=new_assessment_registry,
                            answer=v['value']
                        )
...
class Command(BaseCommand):
    ...
    def map_old_to_new_data(self, assessment_id):
        ...
        meta_data = meta_data_json.get('Background') + meta_data_json.get('Details') + \
            meta_data_json.get('Dates') + meta_data_json.get('Stakeholders')
        metadata_dict = {}
        for d in meta_data:
            k = d['schema']['name']
            v = dict()
        ...
        def _get_bg_crisis_type():
            crisis_type = get_choice_field_key(
                meta_data_json.get('Background'),
                metadata_dict['Crisis Type']['value'],
                AssessmentRegistry.CrisisType
            ) if metadata_dict['Crisis Type']['key'] == 14 or 11 else metadata_dict['Crisis Type']['key']
