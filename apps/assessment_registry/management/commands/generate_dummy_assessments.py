...
class Command(BaseCommand):
    ...
    def generate_leads(self, project: Project, count: int):
        ...
        [LeadPreviewFactory.create(lead=lead) for lead in leads]
    ...
    def generate_assessments(self, project: Project, leads: typing.List[Lead]):
        ...
        organizations = list(Organization.objects.only("id")[:300])
        ...
        for index, lead in enumerate(leads, start=1):
            ...
            if organizations:
                _organizations = random.sample(organizations, random.randint(0, organizations_len))
                stakeholders = []
            ...
            if regions:
                ...
                if geo_areas:
                    geo_areas_filtered = [
                        ...
                        if geo_area.region_id
                        in [region.id for region in selected_regions]  # pyright: ignore [reportGeneralTypeIssues]
                    ]
            ...
            for question_ in random.sample(ary_questions, random.randint(0, ary_questions_len)):
                ...
                AnswerFactory.create(question=question_, **ary_params)
            # Summary
            ...
            if existing_dummy_projects_count:
                ...
                if result == "YES":
                    with transaction.atomic():
                        Project.objects.filter(pk__in=existing_dummy_projects.values("id")).delete()
