...
class AssessmentRegistry(UserResource):
    ...
    protection_info_mgmts = ArrayField(models.IntegerField(choices=ProtectionInfoType.choices), default=list, blank=True)
    protection_risks = ArrayField(models.IntegerField(choices=ProtectionRiskType.choices), default=list, blank=True)
    affected_groups = ArrayField(models.IntegerField(choices=AffectedGroupType.choices), default=list, blank=True)
...
class AdditionalDocument(UserResource):
    ...
    file = models.ForeignKey(File, on_delete=models.SET_NULL, related_name="assessment_reg_file", null=True, blank=True)
...
class ScoreAnalyticalDensity(UserResource):
    class AnalysisLevelCovered(models.IntegerChoices):
        ...
        SEVERITY_OF_SOME_ALL_ISSUE_UNMET_NEEDS_IS_DETAILED = 5, "Severity of some/all issues/unmet_needs_is_detailed"
        ...
        RECOMMENDATIONS_INTERVENTIONS_ARE_PRIORITIZED_RANKED = 10, "Recommendations/interventions are prioritized/ranked"
    ...
    class FigureProvidedByAssessment(models.IntegerChoices):
        ...
        TOTAL_POP_AFFECTED_LIVING_IN_THE_AFFECTED_AREAS = 3, "Total population affected/living in the affected area"
    ...
    assessment_registry = models.ForeignKey(AssessmentRegistry, on_delete=models.CASCADE, related_name="analytical_density")
...
class Question(UserResource):
    ...
    class QuestionSubSector(models.IntegerChoices):
        ...
        BUY_IN_AND_USE_BY_NATIONAL_AND_LOCAL_GOVERNMENT_AGENCIES = 27, "Buy-in and use by national and local government agencies"
...
class Answer(UserResource):
    assessment_registry = models.ForeignKey(AssessmentRegistry, on_delete=models.CASCADE, related_name="answer")
...
class SummarySubPillarIssue(UserResource):
    assessment_registry = models.ForeignKey(
        AssessmentRegistry, on_delete=models.CASCADE, related_name="summary_sub_sector_issue_ary"
    )
    summary_issue = models.ForeignKey("SummaryIssue", on_delete=models.CASCADE, related_name="summary_subsector_issue")
...
class SummarySubDimensionIssue(UserResource):
    assessment_registry = models.ForeignKey(
        AssessmentRegistry, on_delete=models.CASCADE, related_name="summary_focus_subsector_issue_ary"
    )
    sector = models.IntegerField(choices=AssessmentRegistry.SectorType.choices)
    summary_issue = models.ForeignKey(SummaryIssue, on_delete=models.CASCADE, related_name="summary_focus_subsector_issue")
    text = models.TextField(blank=True)
    order = models.IntegerField()
    lead_preview_text_ref = models.JSONField(default=None, blank=True, null=True)
