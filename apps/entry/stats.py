...
def _get_project_geoareas(project):
    qs = (
        ...
        .annotate(extent=Extent("polygons"))
        .values("pk", "admin_level__level", "title", "polygons", "extent", "parent")
    )
    geo_array = []
...
def _get_attribute_widget_value(cd_widget_map, w_value, widget_type, widget_pk=None):
    if widget_type in ["scaleWidget", "multiselectWidget", "organigramWidget"]:
        ...
        context_keys = [f"{widget_pk}-{_value}" for _value in (w_value.keys() if isinstance(w_value, dict) else [])]
        sectors_keys = []
...
def get_project_entries_stats(project, skip_geo_data=False):
    ...
    widgets_pk = [info["pk"] for _info in config.values() for info in (_info if isinstance(_info, list) else [_info])]
    cd_widget_map = {
    ...
    widgets = {widget.pk: widget for widget in Widget.objects.filter(pk__in=widgets_pk, analysis_framework=af)}
        ...
        data.append(
            {
                ...
                "multiselect": {_config["pk"]: collector.get(_config["pk"], []) for _config in w_multiselect_widgets},
                "organigram": {_config["pk"]: collector.get(_config["pk"], []) for _config in w_organigram_widgets},
                "context_sector": {
                    w["pk"]: {
