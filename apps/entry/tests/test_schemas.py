...
class TestEntryQuery(GraphQLTestCase):
    ...
    def test_entry_query_filter(self):
        ...
        for filter_data, expected_entries in [
            ...
            ({"leadPriorities": [self.genum(Lead.Priority.LOW), self.genum(Lead.Priority.HIGH)]}, [entry1_1, entry2_1, entry3_1]),
            ...
            self.assertListIds(
                content["data"]["project"]["entries"]["results"], expected_entries, {"response": content, "filter": filter_data}
            )
            # Lead filter test
            ...
            self.assertListIds(
                content["data"]["project"]["leads"]["results"],
                expected_leads,
...
class TestEntryFilterDataQuery(GraphQLTestCase):
    def setUp(self):
        ...
        self.widget_multiselect = WidgetFactory.create(
            ...
            properties={
                "data": {
                    "options": [
                        {"key": "key-101", "label": "Key label 101"},
                        {"key": "key-102", "label": "Key label 102"},
                        {"key": "key-103", "label": "Key label 103"},
                        {"key": "key-104", "label": "Key label 104"},
                    ]
                },
            },
        )
    ...
    def test(self):
        ...
        EntryAttributeFactory.create(
            entry=entry1_1, widget=self.widget_geo, data={"value": [self.geo_area_3_2.pk]}  # Leaf tagged
        )
        EntryAttributeFactory.create(entry=entry2_1, widget=self.widget_geo, data={"value": [self.geo_area_1.pk]})  # Root tagged
        EntryAttributeFactory.create(
            entry=entry3_1, widget=self.widget_geo, data={"value": [self.geo_area_2_1.pk]}  # Middle child tagged
        )
        EntryAttributeFactory.create(
            entry=entry3_2,
            widget=self.widget_geo,
        ...
        EntryAttributeFactory.create(entry=other_entry, widget=self.widget_multiselect, data={"value": ["key-101", "key-102"]})
        EntryAttributeFactory.create(entry=other_entry, widget=self.widget_number, data={"value": 10002})
        EntryAttributeFactory.create(
            entry=other_entry, widget=self.widget_geo, data={"value": [self.geo_area_3_2.pk]}  # Leaf tagged
        )
        ...
        for filter_name, filter_data, expected_entries in [
            ...
            self.assertListIds(
                content["data"]["project"]["entries"]["results"],
                expected_entries,
