...
from .entry_widget_test_data import ATTRIBUTE_DATA, WIDGET_PROPERTIES
...
class ComprehensiveEntryApiTest(TestCase):
    ...
    def assertAttributeValue(self, widgets_meta, widget_id, widget_properties, attr_data, expected_c_response):
        ...
        if widget_id in (Widget.WidgetType.SCALE,):
            ...
            self.assertTrue(expected_c_response.items() <= c_resposne.items(), (expected_c_response.items(), c_resposne.items()))
        else:
    ...
    @parameterized.expand(
        [
            [widget_id]
            for widget_id, widget_meta in widget_store.items()
