...
from lead.enums import LeadConfidentialityEnum, LeadPriorityEnum, LeadStatusEnum
...
from .models import Entry, EntryComment, ProjectEntryLabel
...
class EntryFilterMixin(django_filters.filterset.FilterSet):
    ...
    comment_created_by = django_filters.ModelMultipleChoiceFilter(
        ...
        queryset=User.objects.all(),
        method="comment_created_by_filter",
    )
    ...
    project_entry_labels = django_filters.ModelMultipleChoiceFilter(
        ...
        queryset=ProjectEntryLabel.objects.all(),
        ...
        if value:
            qs = qs.annotate(
                organization_types=models.functions.Coalesce(
                    "lead__authors__parent__organization_type", "lead__authors__organization_type"
                )
            )
...
def get_filtered_entries_using_af_filter(
    ...
    if project:
        region_max_level = (
            project.regions.annotate(adminlevel_count=models.Count("adminlevel")).aggregate(
                max_level=models.Max("adminlevel_count")
            )["max_level"]
            or 0
        )

    if isinstance(queries, list):
        queries = {q["filter_key"]: q for q in queries}
    elif "filterable_data" in queries:
        # XXX: Pass new structure.
        queries = {q["filter_key"]: q for q in queries["filterable_data"]}
    ...
    for _filter in filters:
        ...
        if not new_query_structure:
            ...
            value_list = [v for v in query.get("value_list") or [] if v is not None]
        ...
        if _filter.filter_type == Filter.FilterType.NUMBER:
            ...
            if value_list:
                ...
                if region_max_level and include_sub_regions and _filter.widget_type == Widget.WidgetType.GEO:
                    ...
                    value_list = (
                        GeoArea.get_sub_childrens(value_list, level=region_max_level)
                        .filter(admin_level__region__project=project)
                        .order_by()
                        .values("admin_level__region__project")
...
def get_entry_filter_object_type(input_type):
    ...
    new_fields_map["filterable_data"] = graphene.List(
        graphene.NonNull(generate_simple_object_type_from_input_type(EntryFilterDataInputType))
    )
