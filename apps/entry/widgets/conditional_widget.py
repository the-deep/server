from analysis_framework.widgets.conditional_widget import (  # type: ignore  # noqa:F401
    WIDGET_ID,
)
...
def update_attribute(widget, data, widget_data):
    ...
    widgets = [w.get("widget") for w in (widget_data.get("widgets") or [])]
...
def get_comprehensive_data(widgets_meta, widget, data, widget_data):
    ...
    selected_widgets = [
        w.get("widget") for w in (widget_data.get("widgets") or []) if w.get("widget", {}).get("key") == selected_widget_key
    ]
