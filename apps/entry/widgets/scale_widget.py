...
def _get_scale(widget, data, widget_properties):
    ...
    scale = next((s for s in options if s["key"] == selected_scale), None)
    scale = scale or {}
    return {
...
def get_comprehensive_data(widgets_meta, widget, data, widget_properties):
    ...
    return {
        **widgets_meta[widget.pk],
        ...
        "label": scale["label"],
        "index": (
            ([(i + 1) for i, v in enumerate(options) if v["key"] == selected_scales[0]] or [None])[0] if selected_scales else None
        ),
    }
