...
def get_valid_geo_ids(raw_values, extract_polygon_title=False):
    ...
    for raw_value in raw_values:
        if is_valid_number(raw_value):
            ...
            geo_areas.extend([int(id) for id in pgeo_areas if is_valid_number(id)])
...
def update_attribute(widget, data, widget_properties):
    ...
    all_values = [*(data.get("value") or []), *(data.get("polygons") or []), *(data.get("points") or [])]
...
def _get_geo_area_parents(geo_areas, admin_levels, geo_area):
    ...
    if p_parent:
        parents.extend(_get_geo_area_parents(geo_areas, admin_levels, p_parent))
    return parents
