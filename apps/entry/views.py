...
from analysis_framework.models import Filter, Widget
...
from deep.permissions import CreateEntryPermission, IsProjectMember, ModifyPermission

from .errors import EntryValidationVersionMismatchError
from .filter_set import EntryCommentFilterSet, EntryFilterSet, get_filtered_entries
from .models import (  # Entry Grouping
    Attribute,
    Entry,
    EntryComment,
    EntryGroupLabel,
...
from .serializers import (  # Entry Grouping
    AttributeSerializer,
...
class EntrySummaryPaginationMixin(object):
    ...
    def get_counts_by_matrix_2d(self, qs):
        ...
        widgets = Widget.objects.filter(
            analysis_framework__project=project, widget_id__in=[matrix1d_widget.WIDGET_ID, matrix2d_widget.WIDGET_ID]
        ...
        filters = {
            ...
            for filter in Filter.objects.filter(
                ...
                key__in=[
                    ...
                    for _key in ([f"{key}-dimensions", f"{key}-sectors"] if widget_id == matrix2d_widget.WIDGET_ID else [key])
                ],
            )
        }
            ...
            **{
                f"{key}__{ele['id' if widget_id == matrix2d_widget.WIDGET_ID else 'key']}__{control_status}": models.Count(
                    ...
                    filter=models.Q(
                        ...
                        filterdata__filter=(filters[f"{key}-{data_type}" if widget_id == matrix2d_widget.WIDGET_ID else key]),
                        filterdata__values__contains=[ele["id" if widget_id == matrix2d_widget.WIDGET_ID else "key"]],
                    ),
                    distinct=True,
                )
                for key, widget_id, properties in widgets
                for data_type in (["sectors", "dimensions"] if widget_id == matrix2d_widget.WIDGET_ID else ["rows"])
                for _ele in properties["data"][data_type]
                for ele in [_ele, *(_ele.get(f"sub{data_type}" if widget_id == matrix2d_widget.WIDGET_ID else "cells") or [])]
    ...
    def get_paginated_response(self, data):
        calculate_summary = self.request.data.get("calculate_summary", self.request.GET.get("calculate_summary", "0")) == "1"
        calculate_count_per_toc_item = (
            self.request.data.get("calculate_count_per_toc_item", self.request.GET.get("calculate_count_per_toc_item", "0"))
            == "1"
        )
        ...
        q = (
            qs.annotate(org=models.functions.Coalesce("lead__authors__parent", "lead__authors"))
            .values("org")
            .annotate(
                org_type=models.functions.Coalesce("lead__authors__parent__organization_type", "lead__authors__organization_type")
            )
            .values("org_type")
            .order_by("org_type")
            .annotate(count=models.Count("org", distinct=True))
        ...
        return response.Response({**super().get_paginated_response(data).data, "summary": summary_data})
...
class EntryViewSet(EntrySummaryPaginationMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated, CreateEntryPermission, ModifyPermission]
    ...
    @action(detail=True, permission_classes=[permissions.IsAuthenticated, ModifyPermission], url_path="control", methods=["post"])
    ...
    @action(
        detail=True, permission_classes=[permissions.IsAuthenticated, ModifyPermission], url_path="uncontrol", methods=["post"]
    )
...
class EntryFilterView(EntrySummaryPaginationMixin, generics.GenericAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            get_filtered_entries(self.request.user, filters)
            .select_related(
                "lead",
                "lead__attachment",
        ...
        if search:
            ...
            fields = TabularField.objects.filter(title__icontains=search, **field_filters)
            queryset = queryset.filter(
                models.Q(lead__title__icontains=search)
                | models.Q(excerpt__icontains=search)
                | (models.Q(tabular_field__in=models.Subquery(fields.values_list("pk", flat=True))))
            )
...
class EntryOptionsView(views.APIView):
    ...
    def get(self, request, version=None):
        ...
        if fields is None or "project_entry_labels" in fields:
            options["project_entry_label"] = ProjectEntryLabel.objects.filter(project__in=projects).values("id", "title", "color")
...
class ProjectEntryLabelViewSet(viewsets.ModelViewSet):
    ...
    def bulk_update_order(self, request, *args, **kwargs):
        ...
        ```
        """
        labels_order = {label["id"]: label["order"] for label in request.data if label.get("id")}
        labels = []
