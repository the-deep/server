...
from user.serializers import (
    ComprehensiveUserSerializer,
    EntryCommentUserSerializer,
...
class AttributeSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = Attribute
        fields = "__all__"
...
class FilterDataSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = FilterData
        fields = "__all__"
...
class ExportDataSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = ExportData
        fields = "__all__"
...
class SimpleFilterDataSerializer(RemoveNullFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = FilterData
        fields = (
            "id",
...
class SimpleExportDataSerializer(RemoveNullFieldsMixin, serializers.ModelSerializer):
    class Meta:
        model = ExportData
        fields = (
            "id",
...
class EntrySerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, DeprecatedUserResourceSerializer):
    ...
    lead_image = serializers.PrimaryKeyRelatedField(required=False, write_only=True, queryset=LeadPreviewImage.objects.all())
    ...
    def validate(self, data):
        ...
        if image:
            if (
                (self.instance and self.instance.image) != image
...
class EditEntriesDataSerializer(RemoveNullFieldsMixin, serializers.ModelSerializer):
    ...
    regions = SimpleRegionSerializer(source="project.regions", many=True, read_only=True)
    ...
    class Meta:
        model = Lead
        fields = ("lead", "entries", "analysis_framework", "geo_options", "regions", "entry_labels", "entry_groups")

    def get_entries(self, lead):
        return EntrySerializer(
            Entry.annotate_comment_count(lead.entry_set),
            many=True,
...
class ComprehensiveAttributeSerializer(
    ...
    def _get_initial_wigets_meta(self, instance):
        projects_id = self.context["queryset"].order_by("project_id").values_list("project_id", flat=True).distinct()
        regions_id = Region.objects.filter(project__in=projects_id).values_list("pk", flat=True)
        geo_areas = {}
        admin_levels = {}

        geo_area_queryset = (
            GeoArea.objects.prefetch_related("admin_level")
            .filter(
...
class EntryGqSerializer(ProjectPropertySerializerMixin, TempClientIdMixin, UserResourceSerializer):
    ...
    image_raw = serializers.CharField(
        ...
        write_only=True,
        help_text=(
            "This is used to add raw base64 images." " This will be changed into gallery image and supplied back in image field."
        ),
    )
    ...
    def validate(self, data):
        ...
        lead = data["lead"]
        if self.instance and lead != self.instance.lead:
            raise serializers.ValidationError({"lead": "Changing lead is not allowed"})
        ...
        # If gallery file is provided make sure user owns the file
        if image:
            if (self.instance and self.instance.image) != image and not image.is_public and image.created_by != request.user:
                raise serializers.ValidationError(
                    {
                        "image": f"You don't have permission to attach image: {image}",
                    }
