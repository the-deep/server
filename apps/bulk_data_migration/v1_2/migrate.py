...
import reversion
from analysis_framework.utils import Widget, update_widgets
from entry.utils import Attribute, update_attributes

from . import excerpt, geo, matrix1d, matrix2d, number_matrix, organigram, scale
...
def migrate_widgets(**kwargs):
    for widget in Widget.objects.filter(**kwargs):
        ...
        if not widget.properties.get("added_from"):
            widget.properties["added_from"] = default_added_from.get(widget.widget_id, "list")
