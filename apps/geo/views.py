...
from .filter_set import AdminLevelFilterSet, GeoAreaFilterSet, RegionFilterSet
from .models import AdminLevel, GeoArea, Region
from .serializers import AdminLevelSerializer, GeoAreaSerializer, RegionSerializer
...
class RegionViewSet(viewsets.ModelViewSet):
    ...
    def get_intersects(self, request, pk=None, version=None):
        ...
        try:
            ...
            raise exceptions.ValidationError(f"Geometry parsed failed, Error: {getattr(e, 'message', repr(e))}")
...
class GeoOptionsView(views.APIView):
    ...
    def get(self, request, version=None):
        ...
        if (
            project.geo_cache_file.name is None
            or project.geo_cache_hash is None
            or project.geo_cache_hash != str(hash(tuple(project.regions.order_by("id").values_list("cache_index", flat=True))))
        ):
            generate_project_geo_region_cache(project)
        return response.Response({"geo_options_cached_file": request.build_absolute_uri(project.geo_cache_file.url)})
...
class GeoAreaView(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return (
            GeoArea.objects.filter(admin_level__region__project=self.kwargs["project_id"], admin_level__region__is_published=True)
            .annotate(
                label=models.functions.Concat(
                    models.F("admin_level__title"), models.Value("/"), models.F("title"), output_field=models.fields.CharField()
                ),
