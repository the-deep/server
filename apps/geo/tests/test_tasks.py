...
class LoadGeoAreasTaskTest(TestCase):
    def setUp(self):
        ...
        admin_level0 = AdminLevel(region=region, parent=None, title="Zone", name_prop="ZONE_NAME", code_prop="HRPCode")
        shape_data = open(
            os.path.join(settings.TEST_DIR, "nepal-geo-json/admin_level2.geo.json"),
            "rb",
        ).read()
        ...
        shape_data = open(
            os.path.join(settings.TEST_DIR, "nepal-geo-json/admin_level3.geo.json"),
            "rb",
        ).read()
    ...
    def test_geojson_api(self):
        ...
        url = "/api/v1/admin-levels/{}/geojson/bounds/".format(self.admin_level0.pk)
