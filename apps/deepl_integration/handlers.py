...
class BaseHandler:
    ...
    def get_callback_url(cls, **kwargs):
        return settings.DEEPL_SERVICE_CALLBACK_DOMAIN + reverse(
            cls.callback_url_name,
            kwargs={
                "version": "v1",
                **kwargs,
            },
...
class AssistedTaggingDraftEntryHandler(BaseHandler):
    ...
    def send_trigger_request_to_extractor(cls, draft_entry):
        ...
        author_organizations = [author.data.title for author in draft_entry.lead.authors.all()]
        payload = {
            "entries": [
                {
        ...
        try:
            response = requests.post(
                DeeplServiceEndpoint.ASSISTED_TAGGING_ENTRY_PREDICT_ENDPOINT, headers=cls.REQUEST_HEADERS, json=payload
            )
    ...
    def _get_or_create_tags_map(cls, tags):
        ...
        def get_tags_map():
            return {tag_id: _id for _id, tag_id in AssistedTaggingModelPredictionTag.objects.values_list("id", "tag_id")}

        current_tags_map = get_tags_map()
        # Check if new tags needs to be created
        new_tags = [tag for tag in tags if tag not in current_tags_map]
        if new_tags:
            # Create new tags
    ...
    def save_data(cls, draft_entry, data):
        ...
        models_version_map = cls._get_or_create_models_version([model_preds["model_info"]])
...
class AutoAssistedTaggingDraftEntryHandler(BaseHandler):
    ...
    def auto_trigger_request_to_extractor(cls, lead):
        ...
        try:
            response = requests.post(
                url=DeeplServiceEndpoint.ENTRY_EXTRACTION_CLASSIFICATION, headers=cls.REQUEST_HEADERS, json=payload
            )
            response_content = response.content
            if response.status_code == 202:
    ...
    def save_data(cls, lead, data_url):
        ...
        for model_preds in data["blocks"]:
            ...
            models_version_map = cls._get_or_create_models_version([data["classification_model_info"]])
...
class LeadExtractionHandler(BaseHandler):
    ...
    def send_trigger_request_to_extractor(
        ...
        try:
            response = requests.post(
                DeeplServiceEndpoint.DOCS_EXTRACTOR_ENDPOINT, headers=cls.REQUEST_HEADERS, data=json.dumps(payload)
            )
            response_content = response.content
            if response.status_code == 202:
                return True
        ...
        logger.error(
            ...
            extra={"data": {"payload": payload, "response": response_content}},
        )
    ...
    def save_data(
        ...
        for image_uri in images_uri:
            ...
            if image_obj:
                lead_image.file.save(os.path.join(image_base_path, os.path.basename(urlparse(image_uri).path)), image_obj)
                lead_image.save()
        lead.update_extraction_status(Lead.ExtractionStatus.SUCCESS)
        return lead
...
class UnifiedConnectorLeadHandler(BaseHandler):
    ...
    def send_retry_trigger_request_to_extractor(
        cls,
        connector_leads_qs: models.QuerySet[ConnectorLead],
        chunk_size=500,
    ) -> int:
        connector_leads = list(  # Fetch all now
            connector_leads_qs.filter(extraction_status=ConnectorLead.ExtractionStatus.RETRYING)
            .only("id", "url")
            .distinct()[:chunk_size]
        )
        ...
        ConnectorLead.objects.filter(pk__in=[c.pk for c in connector_leads]).update(extraction_status=extraction_status)
        return len(connector_leads)

    @classmethod
    def send_trigger_request_to_extractor(cls, connector_leads_qs: models.QuerySet[ConnectorLead]):
        paginator = Paginator(
            connector_leads_qs.filter(extraction_status=ConnectorLead.ExtractionStatus.PENDING)
            .only("id", "url")
            .order_by("id")
            .distinct(),
            100,
...
class NewNlpServerBaseHandler(BaseHandler):
    ...
    @classmethod
    def get_callback_url(cls, **kwargs):
        return settings.DEEPL_SERVER_CALLBACK_DOMAIN + reverse(
            cls.callback_url_name,
            kwargs={
                "version": "v1",
                **kwargs,
    ...
    def send_trigger_request_to_extractor(cls, obj: DeeplTrackBaseModel):
        ...
        payload.update(cls.get_trigger_payload(obj))
        ...
        logger.error(f"{cls.model.__name__} send failed!!", extra={"data": {"context": error_extra_context}})
    ...
    def save_data(
        ...
        if entries_data:
            ...
            new_clusters = TopicModelCluster.objects.bulk_create(
                [TopicModelCluster(topic_model=topic_model, title=_["label"]) for _ in entries_data.values()]
            )
...
class AnalyticalStatementGeoHandler(NewNlpServerBaseHandler):
    ...
    def save_data(
        ...
        if geo_data is not None:
            geo_entry_objs = []
            # Clear out existing
            AnalyticalStatementGeoEntry.objects.filter(task=geo_task).delete()
            existing_entries_id = set(
                Entry.objects.filter(
                    project=geo_task.project, id__in=[int(entry_geo_data["entry_id"]) for entry_geo_data in geo_data]
