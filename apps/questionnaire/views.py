...
from .models import CrisisType, FrameworkQuestion, Question, QuestionBase, Questionnaire
...
from .utils import kobo_toolbox, xls_form
...
class QuestionnaireViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Questionnaire.objects.annotate(
            active_questions_count=models.Count("question", filter=models.Q(question__is_archived=False), distinct=True)
    ...
    def get_options(self, request, version=None):
        options = {
            field: [{"key": key, "value": value} for key, value in values]
            for field, values in (
    ...
    def create_clone(self, request, *args, **kwargs):
        ...
        [
            ...
            for field in ["title", "project_id", "required_duration", "data_collection_technique", "enumerator_skill"]
...
class QuestionViewSet(QuestionBaseViewMixin, viewsets.ModelViewSet):
    ...
    def copy_from_af_question(self, request, *args, **kwargs):
        ...
        new_question = Question.objects.create(
            ...
            order=None,
            **{field.name: getattr(fq, field.name) for field in fq._meta.fields if field.name not in ["id", "order"]},
        )
...
class KoboToolboxExport(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            raise exceptions.ValidationError("Invalid request. Please provide valid XLSForm file and valid access token!!")
