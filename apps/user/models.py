...
from django_otp.plugins import otp_email, otp_static, otp_totp
...
class Profile(models.Model):
        ...
        try:
            ...
            return (
                Feature.objects.filter(
                    Q(is_available_for_all=True) | Q(users=user) | Q(email_domains__domain_name__exact=user_domain)
                )
                .order_by("key")
                .distinct()
            )
    ...
    def is_email_subscribed_for(self, email_type):
        if email_type in self.ALWAYS_SEND_EMAIL_CONDITIONS or (
            email_type in Profile.EMAIL_CONDITIONS_TYPES and email_type not in self.email_opt_outs
        ):
            return True
        return False
...
def get_for_project(project):
    return User.objects.prefetch_related("profile").filter(
        models.Q(projectmembership__project=project) | models.Q(usergroup__in=project.user_groups.all())
    )
...
for label, model, model_admin in OTP_MODELS:
    OTP_PROXY_MODELS.append(
        [
            gen_auth_proxy_model(model, label),
            model_admin,
        ]
    )
