...
from user.models import EmailCondition, Feature, Profile, User
...
from utils.hid.tests.test_hid import HID_EMAIL, HIDIntegrationTest
...
class TestUserSchema(GraphQLTestCase):
    ...
    def test_user_deletion_celery_method(self):
        ...
        users_data = {user.id: _get_user_data(user) for user in all_users}
        anonymized_users_data = {user.id: _get_anonymized_user_data(user) for user in all_users}
