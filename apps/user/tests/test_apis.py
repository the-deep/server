from project.models import Project, ProjectJoinRequest, ProjectMembership
from user.models import EmailDomain, Feature, User
...
class UserApiTests(TestCase):
    def test_active_project(self):
        ...
        ProjectMembership.objects.create(project=project, member=self.user, role=self.admin_role)
    ...
    def test_search_user_without_exclude(self):
        ...
        # in result
        self.create(User, first_name="abc", last_name="xyz", email="something@toggle.com")
    ...
    def test_search_user_with_exclude(self):
        ...
        # Search query is 'search'
        url = "/api/v1/users/?search=search&members_exclude_project=" + str(project.id)
        self.authenticate()
    ...
    def test_notifications(self):
        ...
        request = ProjectJoinRequest.objects.create(project=test_project, requested_by=test_user, role=self.admin_role)
    ...
    def test_password_change(self):
        ...
        data = {"old_password": self.user_password, "new_password": new_pass}
        ...
        data = {"old_password": new_pass, "new_password": "nepa"}  # since password is already changed in the database level
        self.authenticate(user)
        response = self.client.post(url, data)
        self.assert_400(response)

        # now try with posting different `old_password`
        data = {"old_password": "hahahmeme", "new_password": "nepa"}
        self.authenticate(user)
        response = self.client.post(url, data)
        self.assert_400(response)
