...
from .models import ConnectorLead, ConnectorSource, ConnectorSourceLead
...
class UnifiedConnectorLeadsCount(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        connector_leads_qs = (
            ConnectorSourceLead.objects.filter(source__unified_connector__in=keys)
            .order_by()
            .values("source__unified_connector")
            .annotate(
...
class UnifiedConnectorSources(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        connector_source_qs = ConnectorSource.objects.filter(unified_connector__in=keys).order_by("id")
...
class ConnectorSourceLeadsCount(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        connector_leads_qs = (
            ConnectorSourceLead.objects.filter(source__in=keys)
            .order_by()
            .values("source")
            .annotate(
                count=models.Count("connector_lead", distinct=True),
...
class ConnectorSourceLeadLead(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        connector_leads_qs = ConnectorLead.objects.filter(id__in=keys).order_by("id")
        _map = {connector_lead.pk: connector_lead for connector_lead in connector_leads_qs}
        return Promise.resolve([_map[key] for key in keys])


class ConnectorLeadAuthors(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        connector_lead_author_qs = (
            ConnectorLead.objects.filter(id__in=keys, authors__isnull=False)
        ...
        organizations_map = {org.id: org for org in organization_qs}
        return Promise.resolve(
            [[organizations_map.get(author) for author in connector_lead_authors_ids.get(key, [])] for key in keys]
        )
