...
class DraftEntryGqlSerializer(
    ...
    def create(self, data):
        ...
        transaction.on_commit(lambda: trigger_request_for_draft_entry_task.delay(instance.pk))
        return instance
...
class PredictionTagAnalysisFrameworkMapSerializer(TempClientIdMixin, serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if tag is None and not skip_tag:
            raise serializers.ValidationError(dict(tag="Tag is required for this widget."))
        if association is None and not skip_tag:
            raise serializers.ValidationError(dict(association="Association is required for this widget."))
        return data
...
class TriggerDraftEntryGqlSerializer(
    ...
    def create(self, data):
        ...
        transaction.on_commit(lambda: trigger_request_for_auto_draft_entry_task.delay(lead.id))
        return True
