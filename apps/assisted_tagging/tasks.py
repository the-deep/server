...
def sync_tags_with_deepl():
    def _get_existing_tags_by_tagid():
        return {tag.tag_id: tag for tag in AssistedTaggingModelPredictionTag.objects.all()}  # tag_id is from deepl
    ...
    for tag_id, tag_meta in response.items():
        ...
        updated_tags.append(assisted_tag)
    if updated_tags:
        AssistedTaggingModelPredictionTag.objects.bulk_update(updated_tags, fields=("parent_tag",))
