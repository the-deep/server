...
from gallery.serializers import File, SimpleFileSerializer
...
def check_if_url_exists(url, user=None, project=None, exception_id=None, return_lead=False):
    ...
    if not project and user:
        existing_lead = (
            url
            and Lead.get_for(user)
            .filter(
                url__icontains=url,
            )
            .exclude(id=exception_id)
            .first()
        )
    elif project:
        existing_lead = (
...
def raise_or_return_existing_lead(project, lead, source_type, url, text, attachment, return_lead=False):
    ...
    if source_type == Lead.SourceType.WEBSITE:
        ...
        and source_type in [Lead.SourceType.DISK, Lead.SourceType.DROPBOX, Lead.SourceType.GOOGLE_DRIVE]
    ):
...
class LeadSerializer(
    ...
    def validate(self, data):
        ...
        LeadSerializer.add_update__validate(data, self.instance, File.objects.filter(pk=attachment_id).first())
        return data
    ...
    def create(self, validated_data):
        ...
        emm_entities_names = [
            entity["name"] for entity in validated_data.pop("emm_entities", []) if isinstance(entity, dict) and "name" in entity
        ]
...
class LeadPreviewImageSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    """
...
class LeadPreviewSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, serializers.ModelSerializer):
    """
    Serializer for lead preview
    """

    text = serializers.CharField(source="leadpreview.text_extract", read_only=True)
...
class LeadGroupSerializer(RemoveNullFieldsMixin, DynamicFieldsMixin, UserResourceSerializer):
    leads = LeadSerializer(source="lead_set", many=True, read_only=True)
...
class LeadGqSerializer(ProjectPropertySerializerMixin, TempClientIdMixin, UserResourceSerializer):
    ...
    def create(self, validated_data):
        ...
        if lead.connector_lead:
            ...
            LeadExtractionHandler.save_lead_data_using_connector_lead(lead, lead.connector_lead)
...
class LeadCopyGqSerializer(ProjectPropertySerializerMixin, serializers.Serializer):
    ...
    projects = serializers.ListField(child=IntegerIDField(), required=True)
    ...
    def clone_lead(self, original_lead, project_id, user):
        ...
        existing_lead = raise_or_return_existing_lead(
            project_id, new_lead, new_lead.source_type, new_lead.url, new_lead.text, new_lead.attachment, return_lead=True
        )
        if existing_lead:
            return
        ...
        LeadPreviewImage.objects.bulk_create([_get_clone_ready(image, new_lead) for image in preview_images])
        LeadEMMTrigger.objects.bulk_create([_get_clone_ready(emm_trigger, new_lead) for emm_trigger in emm_triggers])
...
class UserSavedLeadFilterSerializer(ProjectPropertySerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, data):
        existing_qs = UserSavedLeadFilter.objects.filter(user=self.current_user, project=self.project).exclude(
            id=self.instance and self.instance.pk
        )
