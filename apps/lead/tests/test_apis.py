...
from entry.models import Entry, EntryGroupLabel, LeadEntryGroup, ProjectEntryLabel
...
class LeadTests(TestCase):
    ...
    def test_options(self):
        ...
        def assert_id(returned_obj, excepted_obj):
            assert set([obj["id"] for obj in returned_obj]) == set([obj["id"] for obj in excepted_obj])
        ...
        # Default options
        DEFAULT_OPTIONS = {
            "confidentiality": [{"key": c[0], "value": c[1]} for c in Lead.Confidentiality.choices],
            "status": [{"key": s[0], "value": s[1]} for s in Lead.Status.choices],
            "priority": [{"key": s[0], "value": s[1]} for s in Lead.Priority.choices],
        }
    ...
    def test_emm_options_get(self):
        ...
        self.create(
            ...
            count=3,
            emm_keyword="keyword1",
            emm_risk_factor="",  # This should not be present as risk factor
        )
        ...
        # Check emm_entities
        assert "emm_entities" in data
        expected_entity_count_set = {(entity1.id, entity1.name, 1), (entity2.id, entity2.name, 2), (entity3.id, entity3.name, 1)}
    ...
    def test_url_exists(self):
        ...
        lead1 = self.create(Lead, source_type="website", project=project, url=common_url)
        lead2 = self.create(Lead, source_type="website", project=project, url="https://different.com/")
        ...
        data = {"title": "Spaceship allegedly spotted in sky"}
        response = self.client.patch(url, data)
        self.assert_200(response)
    ...
    def test_lead_copy(self):
        ...
        # Test Foreign Fields
        self.assertEqual(
            Lead.objects.filter(title=lead1_title).count(), 3, "Should have been 3: Original + Custom created + Copy(of original)"
        )
        self.assertEqual(
            Lead.objects.filter(title=lead1_title).exclude(Q(pk=lead1.pk) | Q(project=project2d)).count(),
            1,
            "Should have been 1: Copy(of original)",
        )
        lead1_copy = Lead.objects.filter(title=lead1_title).exclude(Q(pk=lead1.pk) | Q(project=project2d)).get()
    ...
    def test_lead_filter_with_entries_filter(self):
        ...
        response = self.client.post(url, post_data)
        assert response.json()["count"] == 3
        assert set([each["filteredEntriesCount"] for each in response.json()["results"]]) == set([0, 0, 2]), response.json()
    ...
    def test_get_emm_extra_with_emm_keywords_filter(self):
        ...
        result_triggers = {(x["emm_keyword"], x["emm_risk_factor"], x["total_count"]) for x in data["emm_triggers"]}
    ...
    def test_lead_group_post(self):
        project = self.create_project()
        data = {"project": project.id, "title": "Test Lead Group Title"}
    ...
    def test_lead_group_get(self):
        ...
        self.assertEqual(set(lg["id"] for lg in response.data["results"]), set([leadg_1.id, leadg_2.id]))
    ...
    def test_authors_and_authoring_organization_type(self):
        ...
        # test for authors
        self.assertEqual(set(lead1.get_authors_display()), set(",".join([organization2.title, organization3.title])))
        self.assertEqual(lead2.get_authors_display(), organization1.title)  # organization1 since lead2 authors have parent
        self.assertEqual(lead3.get_authors_display(), organization1.title)
        self.assertEqual(set(lead4.get_authors_display()), set(",".join([organization1.title, organization5.title])))
        ...
        self.assertEqual(lead3.get_authoring_organizations_type_display(), organization_type1.title)
        self.assertEqual(
        ...
        self.assertEqual(lead5.get_authoring_organizations_type_display(), "")

    def test_is_assessment_lead(self):
        project = self.create_project()
        lead = self.create(Lead, project=project, is_assessment_lead=True)
        ...
        data = {"is_assessment_lead": False}
        url = f"/api/v1/leads/{lead.id}/"
        self.authenticate()
        response = self.client.patch(url, data)
        self.assert_400(response)
        self.assertEqual(
            response.data["errors"]["is_assessment_lead"], [ErrorDetail(string="Lead already has an assessment.", code="invalid")]
        )
...
SAMPLE_WEB_INFO_SOURCE = (
    "World Food Programme, UN Children's Fund, Food and Agriculture Organization of the United Nations"  # noqa
)
...
class WebInfoExtractionTests(TestCase):
    ...
    def test_extract_web_info(self):
        ...
        sample_region = self.create(Region, title=SAMPLE_WEB_INFO_COUNTRY, public=True)
        sample_project = self.create(Project, role=self.admin_role)
        sample_project.regions.add(sample_region)

        url = "/api/v1/web-info-extract/"
        data = {"url": SAMPLE_WEB_INFO_URL}
...
class WebInfoDataTestCase(TestCase):
    def test_relief_web(self):
        ...
        data = {"url": SAMPLE_WEB_INFO_URL, "authors_raw": ["Organization1", "Organization2"], "source_raw": "Organization1"}
        self.authenticate()
        response = self.client.post(url, data)
        self.assert_200(response)
        ...
        SAMPLE_AUTO_ASSISTED_TAGGING = {
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "classification": {"1": {"101": {"prediction": 2.0000270270529, "threshold": 0.14, "is_selected": True}}},
                },
                {
                    "type": "text",
                    "page": 3,
