...
class LeadGroup(UserResource):
    ...
    def get_for(user):
        return (
            LeadGroup.objects.filter(models.Q(project__members=user) | models.Q(project__user_groups__members=user))
            .annotate(no_of_leads=models.Count("lead", distinct=True))
            .distinct()
        )
...
class Lead(UserResource, ProjectEntityMixin):
    ...
    author = models.ForeignKey(
        Organization,
        verbose_name="author (legacy)",
        related_name="leads_by_author",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        default=None,
    ...
    connector_lead = models.ForeignKey(
        "unified_connector.ConnectorLead", on_delete=models.SET_NULL, related_name="+", blank=True, null=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not settings.TESTING and (self.id is None or update_fields is None or any(x in update_fields for x in initial_fields)):
            ...
            if (
                not d1
                or d1.get("text") != d2.get("text")
                or d1.get("url") != d2.get("url")
                or d1.get("attachment_id") != d2.get("attachment_id")
            ):
    ...
    def get_for(cls, user, filters=None):
        ...
        qs = (
            ...
            .annotate(
                ...
                view_unprotected=models.F("project__projectmembership__role__lead_permissions").bitand(
                    view_unprotected_perm_value
                ),
                # Get permission value for view permission
                view_all=models.F("project__projectmembership__role__lead_permissions").bitand(view_perm_value),
            )
            .filter(
        ...
        return qs.annotate(
            ...
            filtered_entries_count=(
                models.functions.Coalesce(
                    models.Subquery(
                        get_filtered_entries(user, entries_filter_data)
                        .filter(lead=models.OuterRef("pk"))
                        .values("lead")
                        .order_by()
                ...
                if original_filter
                else models.F("entries_count")
            ),
            controlled_filtered_entries_count=(
                models.functions.Coalesce(
                    models.Subquery(
                        get_filtered_entries(user, entries_filter_data)
                        .filter(lead=models.OuterRef("pk"), controlled=True)
                        .values("lead")
                        .order_by()
                        .annotate(count=models.Count("id"))
                        .values("count")[:1],
                ...
                if original_filter
                else models.F("controlled_entries_count")
            ),
        )
    ...
    def get_authors_display(self, short_name=False):
        ...
        if authors:
            return ",".join([(author.data.short_name if short_name else author.data.title) for author in authors])
        elif self.author:
            # TODO: Remove (Legacy)
    ...
    def get_authoring_organizations_type_display(self):
        authors = self.authors.all()
        if authors:
            return ",".join(set([author.get_organization_type_display() for author in authors if author.data.organization_type]))
        elif self.author:
    ...
    def get_emm_summary(cls, lead_qs):
        # Aggregate emm data
        emm_entities = (
            EMMEntity.objects.filter(lead__in=lead_qs)
            .values("name")
        ...
        emm_triggers = (
            LeadEMMTrigger.objects.filter(lead__in=lead_qs)
            .values("emm_keyword", "emm_risk_factor")
...
class LeadPreview(models.Model):
    ...
    classified_doc_id = models.IntegerField(default=None, null=True, blank=True)
