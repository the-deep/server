...
from .models import EMMEntity, Lead, LeadGroup, LeadPreview, LeadPreviewImage
...
def trigger_lead_extract(modeladmin, request, queryset):
    extract_from_lead.delay(list(queryset.values_list("id", flat=True).distinct()[:10]))
    messages.add_message(
        ...
        mark_safe(
            ...
            + "<br>".join(
                "* {0} : ({1}) {2}".format(*value) for value in queryset.values_list("id", "project_id", "title").distinct()
            )
        ),
    )
