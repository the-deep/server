...
from lead.filter_set import LeadFilterSet, LeadGroupFilterSet
...
from .models import EMMEntity, Lead, LeadEMMTrigger, LeadGroup, LeadPreviewImage
...
valid_lead_url_regex = re.compile(
    ...
    r"^(?:http)s?://"
    # domain...
    r"(?:(?:[A-Z0-9]" r"(?:[A-Z0-9-]{0,61}[A-Z0-9])?\.)+" r"(?:[A-Z]{2,6}\.?|[A-Z0-9-]{2,}\.?)|"
    # localhost...
    r"localhost|"
    # ...or ip
...
class LeadViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        filters = dict()
        filters["entries_filter_data"] = {f[0]: f[1] for f in self.request.data.pop("entries_filter", [])}
        ...
        if lead_id:
            ...
            leads = (
                leads.filter(project=similar_lead.project)
                .annotate(similarity=TrigramSimilarity("title", similar_lead.title))
    ...
    @action(detail=False, permission_classes=[permissions.IsAuthenticated], methods=["get", "post"], url_path="emm-summary")
    def emm_summary(self, request, version=None):
        return self.summary(request, version=version, emm_info_only=False)

    @action(detail=False, permission_classes=[permissions.IsAuthenticated], methods=["get", "post"], url_path="summary")
...
class LeadOptionsView(views.APIView):
    ...
    def get(self, request, version=None):
        ...
        options["emm_entities"] = (
            EMMEntity.objects.filter(lead__project__in=projects)
            .distinct()
            .values("name")
            .annotate(
            ...
            .values("key", "label", "total_count")
            .order_by("name")
        )

        options["emm_keywords"] = (
            LeadEMMTrigger.objects.filter(lead__project__in=projects)
            .values("emm_keyword")
            .annotate(total_count=models.Sum("count"), key=models.F("emm_keyword"), label=models.F("emm_keyword"))
            .order_by("emm_keyword")
        )

        options["emm_risk_factors"] = (
    ...
    @swagger_auto_schema(request_body=LeadOptionsBodySerializer(), responses={200: LeadOptionsSerializer()})
    def post(self, request, version=None):
        ...
        options = {
            ...
            .values("key", "label", "total_count")
            .order_by("name"),
            "emm_keywords": LeadEMMTrigger.objects.filter(emm_keyword__in=emm_keywords, lead__project__in=projects)
            .values("emm_keyword")
            .annotate(total_count=models.Sum("count"), key=models.F("emm_keyword"), label=models.F("emm_keyword"))
            .values("key", "label", "total_count")
            .order_by("emm_keyword"),
            ...
            .order_by("emm_risk_factor"),
            "has_emm_leads": (
                EMMEntity.objects.filter(lead__project__in=projects).exists()
                or LeadEMMTrigger.objects.filter(lead__project__in=projects).exists()
            ),
...
class LeadWebsiteFetch(views.APIView):
    ...
    def website_fetch(self, url):
        ...
        if not valid_lead_url_regex.match(url):
            return response.Response({"error": "Url is not valid"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            # Try with https
            r = requests.head(https_url, headers=DEFAULT_HEADERS, timeout=settings.LEAD_WEBSITE_FETCH_TIMEOUT)
        except requests.exceptions.RequestException:
            https_url = None
            # Try with http
            try:
                r = requests.head(http_url, headers=DEFAULT_HEADERS, timeout=settings.LEAD_WEBSITE_FETCH_TIMEOUT)
            except requests.exceptions.RequestException:
                # doesn't work
                return response.Response(
...
class WebInfoViewMixin:
    ...
    def _process_data(
        ...
        project = None
        if country:
            project = Project.get_for_member(request.user).filter(regions__title__icontains=country).first()
        ...
        organization_context = {
            ...
            "authors": [self.get_organization(author, organization_search) for author in authors_raw],
...
class BaseCopyView(views.APIView):
    ...
    def get_project_ids_with_create_access(cls, request):
        ...
        project_ids = (
            ...
            .filter(add_permission=cls.CLONE_PERMISSION.create)
    ...
    def post(self, request, *args, **kwargs):
        ...
        for entity in entities:
            ...
            edit_or_create_membership = (
                ProjectMembership.objects.filter(
                    member=request.user,
                    project=entity.project,
                )
                .annotate(clone_perm=models.F(self.CLONE_ROLE).bitand(edit_or_create_permission))
                .filter(clone_perm__gt=0)
                .first()
            )
            ...
            for project_id in project_ids:
                ...
                if p_entity:
                    processed_entity_by_project[project_id] = (processed_entity_by_project.get(project_id) or []) + [p_entity.pk]
...
class LeadCopyView(BaseCopyView):
    ...
    def clone_entity(cls, original_lead, project_id, user, context, skip_existing_check=False):
        ...
        LeadPreviewImage.objects.bulk_create([_get_clone_ready(image, lead) for image in preview_images])
        LeadEMMTrigger.objects.bulk_create([_get_clone_ready(emm_trigger, lead) for emm_trigger in emm_triggers])
