...
from .models import Lead, LeadGroup, LeadPreview


class LeadPreviewLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        lead_preview_qs = LeadPreview.objects.filter(lead__in=keys)
        _map = {lead_preview.lead_id: lead_preview for lead_preview in lead_preview_qs}
...
class EntriesCountLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        ...
        stat_qs = (
            Entry.objects.filter(lead__in=keys)
            .order_by("lead")
            .values("lead")
            .annotate(
                total=models.functions.Coalesce(
                    models.Count("id", filter=models.Q(analysis_framework=active_af)),
                    0,
                ),
                controlled=models.functions.Coalesce(
                    models.Count("id", filter=models.Q(controlled=True, analysis_framework=active_af)),
                    0,
                ),
            )
            .values("lead_id", "total", "controlled")
        )
        _map = {stat.pop("lead_id"): stat for stat in stat_qs}
        _dummy = {
            "total": 0,
            "controlled": 0,
        }
...
class LeadGroupLeadCountLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        lead_group_qs = LeadGroup.objects.filter(id__in=keys).annotate(lead_counts=models.Count("lead", distinct=True))
        _map = {id: count for id, count in lead_group_qs.values_list("id", "lead_counts")}
        return Promise.resolve([_map.get(key, 0) for key in keys])


class LeadAuthorsLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        lead_author_qs = Lead.objects.filter(id__in=keys, authors__isnull=False).values_list("id", "authors__id")
        ...
        _map = {org.id: org for org in organization_qs}
        return Promise.resolve([[_map.get(author) for author in lead_author_map.get(key, [])] for key in keys])
...
class LeadAssessmentIdLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        ...
        _map = {lead_id: _id for _id, lead_id in assessments_qs}
        return Promise.resolve([_map.get(key) for key in keys])
...
class LeadDraftEntryCountLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        stat_qs = (
            DraftEntry.objects.filter(lead__in=keys)
            .order_by("lead")
            .values("lead")
            .annotate(
                discarded_draft_entry=models.functions.Coalesce(
                    models.Count("id", filter=models.Q(is_discarded=True)),
                    0,
                ),
                undiscarded_draft_entry=models.functions.Coalesce(
                    models.Count("id", filter=models.Q(is_discarded=False)),
                    0,
                ),
            )
