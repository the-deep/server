...
from entry.models import Attribute, Entry, ExportData, FilterData
...
def get_analysis_framework(lead_id):
    migration = AnalysisFrameworkMigration.objects.filter(old_id=lead_id).first()
    return migration and migration.analysis_framework


def get_region(code):
    migration = CountryMigration.objects.filter(code=code).first()
...
class Command(MigrationCommand):
    def run(self):
        if self.kwargs.get("data_file"):
            ...
            entries = request_with_auth(get_source_url("entries", query="template=1", version="v1"))
    ...
    def import_information(self, entry_data, lead, framework, data):
        ...
        entry.save()
        Entry.objects.filter(id=entry.id).update(created_at=entry_data["created_at"])
    ...
    def migrate_filter_data(self, entry, widget, index=0, number=None, values=None):
    ...
    def migrate_scale(self, entry, widget, element):
        ...
        scale = next((s for s in scale_units if s["key"] == value), None)
        self.migrate_export_data(
            entry,
            widget,
            {
    ...
    def migrate_multiselect(self, entry, widget, element):
        ...
        label_list = []
        for item in value:
            option = next((o for o in options if o["key"] == item), None)
    ...
    def get_number_matrix_export_data(self, attribute, rows, columns):
        ...
        return {"excel": {"values": excel_values}}
    ...
    def get_matrix1d_export_data(self, selections, rows):
        ...
        for selection in selections:
            row = next((r for r in rows if r["key"] == selection["pillar"]), None)
            if not row:
                continue
            cell = next((c for c in row["cells"] if c["key"] == selection["subpillar"]), None)
            if not cell:
                continue
    ...
    def migrate_matrix2d(self, entry, widget, element):
        ...
        filter_values1, filter_values2 = self.get_matrix2d_filter_values(selections)
        self.migrate_filter_data(
            entry,
            widget,
            index=0,
    ...
    def get_matrix2d_export_data(self, selections, data):
        ...
        for selection in selections:
            ...
            dim = next((d for d in dimensions if d["id"] == pillar), None)
            if not dim:
                continue
            sub = next((s for s in dim["subdimensions"] if s["id"] == subpillar), None)
            if not sub:
                continue
            sector = next((s for s in sectors if s["id"] == sector), None)
            if not sector:
                continue

            subsector_names = []
            for subsector in selection.get("subsectors") or []:
                ss = next((ss for ss in sector["subsectors"] if ss["id"] == subsector), None)
                if ss:
