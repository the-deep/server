...
from analysis_framework.models import Section, Widget
from django.core.management.base import BaseCommand
from entry.models import Attribute


def clone_data(src_data, mapping):
    return {key: src_data.get(src_key) for key, src_key, _ in mapping}
...
def multiselect_property_convertor(properties):
    ...
    options = properties if isinstance(properties, list) else properties["options"]
    new_options = []
    option_order = 0
...
def organigram_property_convertor(properties):
    ...
    return {"options": _get_all_new_options(properties)}
    ...
    def get_section_for_af_id(self, af_id):
        if af_id not in self.af_default_sections:
            self.af_default_sections[af_id] = Section.objects.get_or_create(
                ...
                0
            ]  # NOTE: Check if multiple exists if required
