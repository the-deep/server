...
class JwtApiTests(TestCase):
    ...
    def test_login_with_password_greater_than_128_characters(self):
        data = {"username": "Hari@gmail.com", "password": "abcd" * 130}
        url = "/api/v1/token/"
    ...
    def test_valid_login(self):
        ...
        user.save()
        # try to login
        data = {"username": user.username, "password": self.user_password}
        url = "/api/v1/token/"
    ...
    def test_invalid_login_with_password_length_greater_than_128_character(self):
        ...
        user.save()
        # try to login
        data = {"username": user.username, "password": self.user_password * 129}
        url = "/api/v1/token/"
        response = self.client.post(url, data=data)
        self.assert_400(response)
        assert "password" in response.data["errors"]
