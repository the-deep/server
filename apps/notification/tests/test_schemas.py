...
class TestAssignmentQuerySchema(GraphQLTestCase):
    def test_assignments_query(self):
        ...
        entry = EntryFactory.create(analysis_framework=af, lead=lead)
        entry_comment = EntryReviewCommentFactory.create(entry=entry, created_by=user)
        ...
        AssignmentFactory.create_batch(3, project=project, content_object=entry_comment, created_for=user)
        content = _query_check()
    ...
    def test_assignments_with_filter_query(self):
        ...
        AssignmentFactory.create_batch(3, project=project, content_object=lead, created_for=user, is_done=False)
        AssignmentFactory.create_batch(5, project=project, content_object=entry_comment, created_for=user, is_done=True)
