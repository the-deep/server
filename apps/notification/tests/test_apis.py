...
class TestNotificationAPIs(TestCase):
    ...
    def test_update_notification(self):
        ...
        data = [{"id": notifs[0].id, "status": Notification.Status.SEEN}]
    ...
    def test_update_notification_invalid_data(self):
        ...
        data = [
            {"id": notifs[0].id + 1, "status": Notification.Status.SEEN + "a"},
    ...
    def create_join_request(self, project, user=None):
        ...
        join_request = ProjectJoinRequest.objects.create(project=project, requested_by=user, role=self.normal_role)
        return join_request
    ...
    def test_get_filtered_notifications(self):
        ...
        params.update(dict(timestamp__gt=before.strftime("%Y-%m-%d%z"), timestamp__lt=after.strftime("%Y-%m-%d%z")))
        response = self.client.get(url, params)
        self.assert_200(response)
        data = response.json()
