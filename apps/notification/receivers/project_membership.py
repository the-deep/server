...
from project.models import ProjectJoinRequest, ProjectMembership, ProjectRole
...
def create_notification(sender, instance, created, **kwargs):
    ...
    old_notifications = Notification.objects.filter(data__id=instance.id)
...
def update_notification_for_join_request(sender, instance, **kwargs):
    ...
    for admin in admins:
        Notification.objects.create(
            receiver=admin, notification_type=Notification.Type.PROJECT_JOIN_REQUEST_ABORT, project=instance.project, data=data
        )


@receiver(pre_save, sender=ProjectMembership)
def remove_notifications_for_former_project_admin(sender, instance, **kwargs):
    admin_roles = ProjectRole.get_admin_roles()

    try:
        old_membership = ProjectMembership.objects.get(id=instance.id)

        if old_membership.role in admin_roles and instance.role not in admin_roles:
            old_notifications = Notification.objects.filter(receiver=instance.member, project=instance.project)
            old_notifications.delete()

        if old_membership.role not in admin_roles and instance.role in admin_roles:
            ...
            for old_project_join_request in old_project_join_requests:
                Notification.objects.create(
                    ...
                    notification_type=Notification.Type.PROJECT_JOIN_REQUEST,
                    project=instance.project,
                    data=ProjectJoinRequestSerializer(old_project_join_request).data,
                )
...
def create_notifications_for_new_project_admin(sender, instance, created, **kwargs):
    if created is True:
