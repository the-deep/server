...
class NotificationFilterSet(django_filters.FilterSet):
    ...
    is_pending = django_filters.ChoiceFilter(label="Action Status", method="is_pending_filter", choices=BOOLEAN_CHOICES)
    ...
    def is_pending_filter(self, queryset, name, value):
        if value == self.TRUE:
            ...
            return queryset.filter(~Q(data__status="pending") | Q(data__status__isnull=True)).distinct()
        return queryset
