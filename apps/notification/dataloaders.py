...
class AssignmentLoader(DataLoaderWithContext):
    def batch_load_fn(self, keys):
        assignment_qs = list(Assignment.objects.filter(id__in=keys).values_list("id", "content_type__model", "object_id"))
        ...
        for _id, title in Lead.objects.filter(id__in=leads_id).values_list("id", "title"):
            _lead_id_map[_id] = dict(id=_id, title=title)

        _entry_review_comment_id_map = {}

        for _id, entry_id, lead_id in EntryReviewComment.objects.filter(id__in=entry_review_comment_id).values_list(
            "id", "entry__id", "entry__lead_id"
        ):
            _entry_review_comment_id_map[_id] = dict(id=_id, entry_id=entry_id, lead_id=lead_id)

        _result = {
            _id: {
                "content_type": content_type,
                "lead": (_lead_id_map.get(object_id) if content_type == get_model_name(Lead) else None),
                "entry_review_comment": (
                    _entry_review_comment_id_map.get(object_id) if content_type == get_model_name(EntryReviewComment) else None
                ),
            }
