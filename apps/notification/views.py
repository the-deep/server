...
from notification.filter_set import AssignmentFilterSet, NotificationFilterSet
...
from .serializers import AssignmentSerializer, NotificationSerializer
...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def get_count(self, request, version=None):
        ...
        unseen_notifications = qs.filter(status=Notification.Status.UNSEEN)
...
class AssignmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            Assignment.get_for(self.request.user)
            .select_related(
                "project",
                "created_by",
                "content_type",
    ...
    @action(detail=False, methods=["POST"], permission_classes=[permissions.IsAuthenticated], url_path="bulk-mark-as-done")
