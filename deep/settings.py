...
from utils.aws import fetch_db_credentials_from_secret_arn
from utils.aws import get_internal_ip as get_aws_internal_ip
...
if IN_AWS_COPILOT_ECS and env("SERVICE_ENVIRONMENT_TYPE") == "web":
    ALLOWED_HOSTS.append(get_aws_internal_ip(env("SERVICE_ENVIRONMENT_TYPE")))

# Database
# https://docs.djangoproject.com/en/1.11/ref/settings/#databases
if IN_AWS_COPILOT_ECS:
    DBCLUSTER_SECRET = env.json("DEEP_DATABASE_SECRET") or fetch_db_credentials_from_secret_arn(env("DEEP_DATABASE_SECRET_ARN"))
    DATABASES = {
        "default": {
...
if env("DJANGO_USE_S3"):
    ...
    AWS_S3_SECRET = env.json("DEEP_BUCKET_ACCESS_USER_SECRET") or (
        env("DEEP_BUCKET_ACCESS_USER_SECRET_ARN")
        and fetch_db_credentials_from_secret_arn(env("DEEP_BUCKET_ACCESS_USER_SECRET_ARN"), ignore_error=True)
    )
    if AWS_S3_SECRET:
